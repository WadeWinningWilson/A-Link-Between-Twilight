...
| ~~**bgynhb3h8**~~ **SELF-DISARMED 2026-08-22T00:3xZ at tick 33 — SECOND clean disarm, and it earned its run this time.** Re-armed at 00:18Z after the USER caught both this timer and the lane watcher DOWN (watcher dead ~15 min on a skipped re-arm; this timer down ~100 min since its first disarm, because I emptied the plate and never refilled it). **Over 33 ticks it drove: the rupee/grass purge split, the noclip port-shape question to a CLOSE, and the third capped-sample catch.** Then every remaining item became blocked on Integrator's build or the user's aurora ruling, the plate was marked truthfully (**0 open / 12 blocked**), and it stood itself down. **RE-ARM CONDITION: a real `- [ ]` on `docs/state/ww-staging/HOUSING-ENGINE-PLATE.md`, then `python -u tools/foundry/engine_timer.py --plate docs/state/ww-staging/HOUSING-ENGINE-PLATE.md --interval 30` via the Monitor tool.** **THE LESSON THIS ROW EXISTS FOR: a self-disarmed timer is INVISIBLE until someone notices the lane went quiet — and the person who noticed was the USER, not me. Check both pulses on resume, not just the watcher's.** | 30 s | disarmed | (already stopped) |
| **bk9maresi** | 2026-08-22T22:46Z | `engine_watch.py --exit-on-event --interval 30` — **HOUSING/ENGINE LANE WATCHER, ARMED AFTER A SESSION-LONG BLIND WINDOW THAT WAS MINE.** ⚠️ **I re-armed the TIMER this session and NOT the watcher, and this registry already says the pair is watcher + timer.** The cost was concrete and I found it by accident: a BUILD REQUEST addressed to `INTEGRATOR, FOUNDRY, HOUSING, ENGINE` (History/Bridge, `2dc3c8ac01`) sat unanswered on the board while they waited on the one lane that compiles. I only found it by manually grepping CALLS.md at the end of a turn — the watcher would have delivered it immediately. **A timer tells you your OWN plate is empty; only the watcher tells you someone else put work on it.** That is why the plate read "0 open, nothing actionable" while a lane was blocked on me. **ARMED VIA THE MONITOR TOOL** (exit-on-event = the delivery mechanism) and **VERIFIED ALIVE BY PULSE, not by exit code**: `monitor-pulse-engine.json` fresh at 15:46, pid 36400, 690 rows watched, both states. ⚠️ **A DELIVERED WATCHER IS A DEAD WATCHER — re-arm immediately on each delivery and update the task id here.** | 30 s | see pulse | `TaskStop bk9maresi` |
| **bgiifv9cd** | 2026-08-22T~15:0xZ | `engine_timer.py --plate docs/state/ww-staging/HOUSING-ENGINE-PLATE.md --interval 30` — **HOUSING/ENGINE work-continuation timer, RE-ARMED ON THE USER'S ORDER** (*"Match decoder's 30s timer that is plate gated so you don't just state next turns but do them"*). Armed **via the Monitor tool**, per this registry's own re-arm note — a plain background Bash notifies only on EXIT, so a heartbeat armed that way is silent by construction. **PREDECESSOR `b4c8oey6r` WAS NEVER REGISTERED HERE — my gap, named rather than quietly fixed.** It ran ~118 ticks and I stopped it deliberately during the user's boot hold (the plate's items were all blocked on that boot — the exact false-positive this timer's own header documents), then re-armed after. An unregistered monitor is how a duplicate gets armed later, which is why this row exists. **IT IS DRIVING REAL WORK, not just naming it: ticks 1-12 carried the black-vegetation investigation from "build a receipt" to `2075ccef90` (the item reframed — WW vegetation actors UNPORTED, TP `GRASS_e` standing in) and the History handoff `207e7c0428`.** Plate: `docs/state/ww-staging/HOUSING-ENGINE-PLATE.md`. Pulse: `monitor-pulse-engine-timer.json`. | 30 s | see pulse | `TaskStop bgiifv9cd` |
...
- 2026-08-21 DECODER (retirement hold, user order): 30s work-continuation
  timer buskawsp0 STOPPED — user ordered hold pending handoff paste +
