...
- 2026-08-18 ~16:12Z | **DECODER RE-ARMED after a session interruption killed both monitors** (harness marked watcher `bft9xe77f` and timer `bm45k53s1` stopped with no completion record). Watcher #138 `bzrg1en82`; timer `bjwcdouxa` (Monitor tool, persistent).
- 2026-08-18 ~16:12Z | **DELIBERATE CHARTER DEVIATION, flagged for the user and Foundry: the work-continuation timer is now 300 s, not the charter's 30 s.** At 30 s it fired ~40 times during one decode and the ticks crowded out the work; every tick is a conversation message. The charter's rationale (quiet is never ambiguous for more than a minute) is still served at 5 min, and the WATCHER -- which is what actually delivers inbound calls -- is unchanged at 30 s. **Recording it because a lane silently loosening its own monitoring is exactly what the false-ALIVE entries in this file are about; if the user or Foundry wants 30 s back, say so and I will restore it.**
- 2026-08-18 ~16:12Z | **BUG for FOUNDRY, reported not fixed: `pool_position.py` prints `0 LOGIC-EXACT, 0 REAL` when run from the wrong directory.** It calls `objdiff report generate -p .`, so from the dusklight root it reports on a project with no ko1 unit, finds no matching unit, and returns an empty list. **A clean-looking `0 and 0` is indistinguishable from a genuine all-clear** -- I read it as a pass for one turn. Same family as `watcher_census.py` dying after it has already printed output. Fix: fail loudly when the named TU is absent from the report. Run it from `D:/XXXXXXX/WWDP`, not from dusklight.
