...
PULSE = os.path.join(ROOT, "tools", "foundry", "monitor-pulse-integrator.json")
...
def main():
    ...
    tick = 0
    print("INTEGRATOR-TIMER armed interval=%ds plate=%s"
    ...
    while True:
        ...
        if open_items is None:
            ...
            print("INTEGRATOR-TICK %d - %d open, %d done. NEXT: %s"
                  % (tick, len(open_items), done, open_items[0]), flush=True)

        if args.max_ticks and tick >= args.max_ticks:
            print("INTEGRATOR-TIMER max-ticks reached, exiting.", flush=True)
            return 0
