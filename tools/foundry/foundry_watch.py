#!/usr/bin/env python3
# ============================================================================
# foundry_watch.py - the FOUNDRY lane monitor. Recorded in-tree at arm time
...
#
#   ARM:  python -u tools/foundry/foundry_watch.py --exit-on-event
#   Then IMMEDIATELY: record the task ID in MONITOR-REGISTRY.md, verify with
#   `monitor_pulse.py tools/foundry/monitor-pulse-foundry.json` (expect ALIVE,
#   new pid), and run `watcher_census.py` (expect this watcher as a leaf).
#
# CHARTER (ported from Decoder, DECODER-BRIEFING.md S3b.1): interval defaults to 30 s - with a 30 s
...
#   5. ONE PULSE FILE PER LANE - monitor-pulse-foundry.json, nobody else's.
...
# source field names FOUNDRY are this lane's own filings and do not wake it -
# UNLESS the addressee field also names FOUNDRY, because a row addressed to
...
PULSE = HERE / "monitor-pulse-foundry.json"
...
# Trailing source field. **THIS IS THE LINE THE ENGINE PORT GOT WRONG AND
# SHIPPED SUPPRESSING ZERO ROWS.** Decoder rows end `| DECODER | date`, so a
# pattern requiring the lane name ALONE works there. FOUNDRY rows do not:
# they end `| Foundry, monitor census, pid-verified... | 2026-08-22` -- lane
# name, comma, free text, THEN the date. A DECODER-shaped pattern matches
# none of them, so every self-filing would wake this lane.
# Non-vacuity is MEASURED before arming, never assumed.
SELF_TAIL = re.compile(r"\|\s*Foundry\b[^|]*(\|[^|]*)?$", re.I)
...
USAGE = """foundry_watch.py - the Foundry lane monitor, with a proof of life.

  foundry_watch.py [--interval SEC] [--pulse PATH] [--exit-on-event]

  --interval SEC     board check cadence (default 30, per DECODER-BRIEFING, ported)
  --pulse PATH       pulse file (default tools/foundry/
                     monitor-pulse-foundry.json). ONE PULSE FILE PER LANE.
...
def main():
    ...
    emit("FOUNDRY-WATCH ARMED %s | watching %d row(s) (both states) for "
         "FOUNDRY/ALL LANES | interval %ds | pulse: %s"
        ...
        try:
            ...
            if rows is None:
                ...
                if fresh:
                    ...
                    for v in fresh[:4]:
                        emit("FOUNDRY-WATCH ROW | %s" % v)
                    if len(fresh) > 4:
                        emit("FOUNDRY-WATCH (+%d more row(s))"
                             % (len(fresh) - 4))
                seen = rows
