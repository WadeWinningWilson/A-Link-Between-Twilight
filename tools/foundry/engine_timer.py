...
#
# Copied from DECODER's timer on the user's order 2026-08-17 ("copy decoder's
...
#       python -u tools/foundry/integrator_timer.py --interval 180
#
# ============================================================================
...
DEFAULT_PLATE = os.path.join(ROOT, "docs", "state", "ww-staging", "INTEGRATOR-PLATE.md")
# Copied verbatim from integrator_timer.py (itself Decoder's timer) on the
# user's order: MATCH DECODER'S MONITOR + TIMER. The ONLY change is this
# path: the original writes monitor-pulse-integrator.json, and two lanes
# sharing one pulse file makes each read the other's liveness as its own.
PULSE = os.path.join(ROOT, "tools", "foundry", "monitor-pulse-engine-timer.json")
...
def main():
    ...
    tick = 0
    print("ENGINE-TIMER armed interval=%ds plate=%s"
    ...
    while True:
        ...
        if open_items is None:
            ...
            print("ENGINE-TICK %d - PLATE FILE MISSING (%s). Not treating as empty; "
                  "this is a fault, not quiet." % (tick, args.plate), flush=True)
        elif not open_items:
            pulse(args.interval, tick, "EMPTY-DISARMED", None)
            print("ENGINE-TIMER disarming: plate EMPTY after %d done. A timer that "
            ...
            print("ENGINE-TICK %d - %d open, %d done. NEXT: %s"
                  % (tick, len(open_items), done, open_items[0]), flush=True)

        if args.max_ticks and tick >= args.max_ticks:
            print("ENGINE-TIMER max-ticks reached, exiting.", flush=True)
            return 0
