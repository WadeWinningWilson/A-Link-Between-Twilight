...
// ALBT keeps its own load-order Mods page (ui/mods.hpp) instead of main's mods_window.hpp.
...
void ModLoader::try_load_mod(
    ...
    // Native loading extracts from the bundle, touches the filesystem and runs the library's
    // static initializers; a throw from any of those must fail this mod, not the process.
    try {
        if (load_native_if_present(mod) && mod.native) {
            mod.manifestInfo = build_manifest_info(mod.native->parsed);
        }
    } catch (const std::exception& e) {
        fail_mod(mod, MOD_ERROR, fmt::format("Native load failed: {}", e.what()));
    } catch (...) {
        fail_mod(mod, MOD_ERROR, "Unknown exception during native load");
    }
