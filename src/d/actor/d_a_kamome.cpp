...
// KIT-DONOR: d/actor/d_a_kamome.cpp
// KIT-DONOR-REF: zeldaret/tww@1d57f0468986987ec26a3d1800bdc1aaad3794db
// KIT-DONOR-STATUS: Equivalent
/**
 * d_a_kamome.cpp
 * NPC - Seagull
 *
 * ============================================================
 * §232 WW seagull DIRECT PORT (donor: D:/XXXXXXX/WW DP/src/d/actor/d_a_kamome.cpp).
 * Logic is VERBATIM from the donor — "we ALWAYS do what the donor does". Only the
 * pig (d_a_kb) direct-port crash recipes + identifier renames are applied, marked
 * §232 at each site:
 *   1. Model = raw at consume (DN-3): the BDL goes through
 *      dExtNpcMount_acquireModelData("Kamome","ka.bdl") (parse-at-consume); anims
 *      (BCK/BAS) stay raw getObjectRes (the morf binder fixes them).
 *   2. Render: the port SPLIT donor updateDL() into modelCalc()+entryDL(); the
 *      whole-model joint calc is modelCalc() (donor's mpMorf->calc() maps to it),
 *      and the draw does dComIfGd_setList() before entryDL().
 *   3. 64-bit: setUserArea((u32)ptr) -> (uintptr_t)ptr.
 *   4. dComIfG_Bgsp() returns a REFERENCE in the port (-> becomes . ; *Bgsp() drops).
 *   5. dComIfGd_setShadow gains 3 trailing port params (0, 1.0f, getSimpleTex()).
 *   6. fopAcM_ct_Retail/_Demo collapse to a single fopAcM_ct; cPhs_State->cPhs_Step;
 *      dComIfG_resDeleteDemo->dComIfG_resDelete; DEMO_SELECT collapses to retail;
 *      fopAcM_create gains the port's explicit trailing (angle,scale,arg) args.
 * ============================================================
 */
...
// §232 direct-port support headers (mirror the pig d_a_kb):
#include "d/d_ext_npc_mount.h"           // §232 DN-3 parse-at-consume model fixup (acquireModelData)
#include "d/d_ext_ww_actor_shims.h"      // §232 seagull donor-gap shims (daSea_*/att_chkEnemySound/SFX/cCcD)
...
// is the SAME physical array WW indexed by number; the port only renamed the slots
// by developer (TREG=[0], YREG=[6], XREG=[17], ...). Re-supply WW's REG0/REG6/REG17
// against those exact slots so the seagull's tuning knobs stay byte-faithful; in a
// non-REGHIO build they collapse to 0/0.0f exactly like the port's own reg macros.
// ============================================================
#if ENABLE_REGHIO
#define REG0_F(i)  g_regHIO.mChildReg[0].mFloatReg[i]
#define REG0_S(i)  g_regHIO.mChildReg[0].mShortReg[i]
#define REG6_F(i)  g_regHIO.mChildReg[6].mFloatReg[i]
#define REG6_S(i)  g_regHIO.mChildReg[6].mShortReg[i]
#define REG17_F(i) g_regHIO.mChildReg[17].mFloatReg[i]
#define REG17_S(i) g_regHIO.mChildReg[17].mShortReg[i]
#else
#define REG0_F(i)  (0.0f)
#define REG0_S(i)  (0)
#define REG6_F(i)  (0.0f)
#define REG6_S(i)  (0)
#define REG17_F(i) (0.0f)
#define REG17_S(i) (0)
#endif
...
void anm_init(kamome_class* i_this, int anmResIdx, float morf, unsigned char loopMode, float speed, int soundAnmResIdx) {
    ...
    // §232 DN-3: anims (BCK/BAS) stay RAW getObjectRes — setAnm's binder fixes them.
...
void* s_a_d_sub(void* ac1, void* ac2) {
    ...
    if ((esa_check_count < 100) && fopAc_IsActor(ac1) && fpcM_GetName(ac1) == fpcNm_ESA_e) {
        ...
        if (esa->field_0x298 == 0) {
            ...
            // §232 port dComIfG_Bgsp() returns a reference (-> becomes .)
...
// §232 port J3DJointCallBack is int(*)(J3DJoint*,int) — donor took J3DNode*; adapt
// the parameter type (the body already casts to J3DJoint*), matching the pig.
static BOOL nodeCallBack(J3DJoint* node, int calcTiming) {
...
static BOOL daKamome_Draw(kamome_class* i_this) {
    ...
    i_this->mShadowId = dComIfGd_setShadow(
        ...
        i_this->mAcch.m_gnd,
        &i_this->actor.tevStr,
        // §232 port setShadow adds 3 trailing params (shadow-tex); standard actor
        // values, matches d_a_kb / d_a_b_dr / alink.
...
void kamome_bgcheck(kamome_class* i_this) {
    ...
    // §232 port dComIfG_Bgsp() returns a reference (donor deref *Bgsp() drops)
...
#endif
    {
        if (dComIfGp_checkPlayerStatus0(0, daPyStts0_SHIP_RIDE_e) && (i_this->mGlobalTimer & 0x7f) == 0) {
            ...
            if (ko < i_this->mKoMaxCount) {
                ...
                sp08.y += 4000.0f;
                // §232 port fopAcM_create has no default trailing args — pass WW
                // defaults explicitly (angle=NULL, scale=NULL, argument=0).
                fopAcM_create(fpcNm_KAMOME_e, ko << 8 | 0xffff0007, &sp08, fopAcM_GetRoomNo(&i_this->actor), NULL, NULL, 0);
            }
        }
    }
}
...
void kamome_imouto_move(kamome_class* i_this) {
    ...
    // §232 port dComIfG_Ccsp() returns a POINTER (donor -> kept)
...
void daKamome_setMtx(kamome_class* i_this) {
    ...
    // §232 render (recipe #2): donor mpMorf->calc() = whole-model joint calc; the
    // port's calc() is the per-joint MtxCalc callback, so use modelCalc() (which does
    // setFrame + setMtxCalc + model->calc) with the head callback registered.
...
static BOOL daKamome_Delete(kamome_class* i_this) {
    // §232 port lacks dComIfG_resDeleteDemo — use the standard resLoad counterpart.
    dComIfG_resDelete(&i_this->mPhase, "Kamome");
    if (i_this->m688 != 0) {
        hio_set = false;
        mDoHIO_DELETE_CHILD(l_kamomeHIO.mNo);
    }
    return TRUE;
}
...
static BOOL createHeap(fopAc_ac_c* a_this) {
    ...
    // §232 DN-3 (recipe #1): the BDL is RAW at getObjectRes — route it through the
    // mount's parse-at-consume acquireModelData; anims stay raw. NULL-guard -> a
    // graceful missing seagull beats a crash.
    J3DModelData* pModelData = dExtNpcMount_acquireModelData("Kamome", "ka.bdl");
    if (pModelData == NULL) {
        return FALSE;
    }
    ...
    // §232 64-bit fix (recipe #3): donor cast (u32)&actor truncates the pointer on
    // x86_64 -> nodeCallBack reads back a wild ptr via getUserArea(). Store uintptr_t.
    i_this->mpMorf->getModel()->setUserArea((uintptr_t)&i_this->actor);
    return TRUE;
}

/* 00004B94-00004F84       .text daKamome_Create__FP10fopAc_ac_c */
static cPhs_Step daKamome_Create(fopAc_ac_c* a_this) {
    ...
    kamome_class* i_this = (kamome_class*)a_this;
    // §232 fopAcM_ct_Retail/_Demo version-split collapses to a single fopAcM_ct.
    fopAcM_ct(a_this, kamome_class);

    cPhs_Step PVar1 = dComIfG_resLoad(&i_this->mPhase, "Kamome");
    if (PVar1 == cPhs_COMPLEATE_e) {
        ...
        // §232 DEMO_SELECT collapses to the retail heap size.
        if (!fopAcM_entrySolidHeap(a_this, createHeap, 0x1360)) {
            return cPhs_ERROR_e;
        }
        ...
        if (!hio_set) {
            l_kamomeHIO.mNo = mDoHIO_CREATE_CHILD("カモメ", &l_kamomeHIO);
            i_this->m688 = 1;
            hio_set = true;
        }
