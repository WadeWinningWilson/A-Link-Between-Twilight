/**
 * @file d_a_albw_shade_watcher.cpp
 *
 * ============================================
 * NEW CODE — ALBW Port — "Shade's Refuge"
 * Shade Watcher: a stationary clone of the Hero's-Shade golden wolf
 * (d_a_npc_gwolf) used as a Lies of P-style rest/respawn/warp point.
 * Slice 1: copied verbatim from d_a_npc_gwolf for a guaranteed-correct
 * sit-and-breathe visual; the howl/warp/talk/questline behaviour is
 * stripped in a follow-up pass.
 * ============================================
*/

#include "d/dolzel_rel.h" // IWYU pragma: keep

#include "d/actor/d_a_albw_shade_watcher.h"
#include "d/actor/d_a_npc.h"
#include "Z2AudioLib/Z2Instances.h"
#include "d/d_camera.h"
#include "SSystem/SComponent/c_math.h"  // Shade's Refuge: cM_ssin/cM_scos for camera snap
...
static ALBW_SHADE_WATCHER_HIO_CLASS l_HIO;
...
char DUSK_CONST* DUSK_CONST daAlbwShadeWatcher_c::mEvtCutNameList[5] = {
    "",
    "ATTACK_WARP",
    "ATTACK_WARP_HORSE",
    "HOWLING_SESSIONA",
...
daAlbwShadeWatcher_c::cutFunc DUSK_CONST daAlbwShadeWatcher_c::mEvtCutList[5] = {
    NULL,
    &daAlbwShadeWatcher_c::ECut_attackWarp,
    &daAlbwShadeWatcher_c::ECut_attackWarpHorse,
    &daAlbwShadeWatcher_c::ECut_howlingSessionA,
    &daAlbwShadeWatcher_c::ECut_meetGWolf,
};

daAlbwShadeWatcher_HIOParam const daAlbwShadeWatcher_Param_c::m = {
    30.0f,
    -4.0f,
    1.0f,
    600.0f,
...
#if DEBUG
daAlbwShadeWatcher_HIO_c::daAlbwShadeWatcher_HIO_c() {
    m = daAlbwShadeWatcher_Param_c::m;
}

void daAlbwShadeWatcher_HIO_c::listenPropertyEvent(const JORPropertyEvent* event) {
    // NONMATCHING
}

void daAlbwShadeWatcher_HIO_c::genMessage(JORMContext* ctx) {
    // NONMATCHING
}
#endif

daAlbwShadeWatcher_c::daAlbwShadeWatcher_c() {}
...
cPhs_Step daAlbwShadeWatcher_c::create() {
    fopAcM_ct(this, daAlbwShadeWatcher_c);
...
int daAlbwShadeWatcher_c::CreateHeap() {
...
int daAlbwShadeWatcher_c::Delete() {
    fopAcM_GetID(this);
    this->~daAlbwShadeWatcher_c();
    return 1;
}

int daAlbwShadeWatcher_c::Execute() {
    // ============================================
    // NEW CODE — ALBW Port — "Shade's Refuge"
    // On a death-respawn at this watcher, Link arrives via a point -1 (coords)
    // spawn that carries no camera data, so the camera stays where it was at
    // death. Armed by the game-over respawn branch, snap it behind Link for a
    // few frames (overrides the room's initial camera) so the return is framed.
    // ============================================
    if (dShadeRefuge_consumeRespawnCamera()) {
        fopAc_ac_c* link = (fopAc_ac_c*)daPy_getPlayerActorClass();
        dCamera_c* cam = dCam_getBody();
        if (link != NULL && cam != NULL) {
            const f32 kDist = 420.0f, kEyeH = 200.0f, kLookH = 140.0f;
            const s16 a = link->shape_angle.y;
            const f32 fwdX = cM_ssin(a);
            const f32 fwdZ = cM_scos(a);
            cXyz center(link->current.pos.x, link->current.pos.y + kLookH, link->current.pos.z);
            cXyz eye(link->current.pos.x - fwdX * kDist, link->current.pos.y + kEyeH,
                     link->current.pos.z - fwdZ * kDist);
            cam->Reset(center, eye);
        }
        // Top off the small fall damage the getup spawn applied, so the respawn
        // lands Link at full health despite the recovery animation.
        dComIfGs_setLife(dComIfGs_getMaxLife());
    }
    return execute();
}

int daAlbwShadeWatcher_c::Draw() {
    J3DModelData* mdlData_p = mAnm_p->getModel()->getModelData();
    mdlData_p->getMaterialNodePointer(2)->setMaterialAnm(mpMatAnm);

    return draw(
        chkAction(&daAlbwShadeWatcher_c::test), FALSE, mpHIO->m.common.real_shadow_size, NULL, 0
    );
}

int daAlbwShadeWatcher_c::draw(int param_1, int param_2, f32 param_3, GXColorS10* i_color, int param_5) {
    J3DModel* model = mAnm_p->getModel();
...
int daAlbwShadeWatcher_c::ctrlJoint(J3DJoint* i_joint, J3DModel* i_model) {
...
int daAlbwShadeWatcher_c::createHeapCallBack(fopAc_ac_c* a_this) {
    return static_cast<daAlbwShadeWatcher_c*>(a_this)->CreateHeap();
}

int daAlbwShadeWatcher_c::ctrlJointCallBack(J3DJoint* i_joint, int param_2) {
    if (param_2 == 0) {
        J3DModel* model = j3dSys.getModel();
        daAlbwShadeWatcher_c* i_this = (daAlbwShadeWatcher_c*)model->getUserArea();
...
void daAlbwShadeWatcher_c::setParam() {
    ...
    // ============================================
    // NEW CODE — ALBW Port — "Shade's Refuge"
    // The watcher is interactable as HUMAN Link (and wolf). The golden wolf
    // gates its talk/lock-on attention behind wolf form (checkNowWolf); we
    // keep SPEAK|TALK enabled in every form so the talk prompt always shows.
    // (was: if (!daPy_py_c::checkNowWolf()) { flag = 0; })
    // ============================================
...
BOOL daAlbwShadeWatcher_c::main() {
    if (!doEvent()) {
        doNormalAction(1);
    }
...
BOOL daAlbwShadeWatcher_c::ctrlBtk() {
    if (mpMatAnm != NULL) {
...
void daAlbwShadeWatcher_c::adjustShapeAngle() {
    s16 target = 0;
...
void daAlbwShadeWatcher_c::setAttnPos() {
...
bool daAlbwShadeWatcher_c::setExpressionAnm(int i_index, bool i_modify) {
    return true;
}

void daAlbwShadeWatcher_c::setExpression(int i_index, f32 i_morf) {
    /* empty function */
}

void daAlbwShadeWatcher_c::setMotionAnm(int i_index, f32 i_morf) {
...
void daAlbwShadeWatcher_c::setMotion(int i_motion, f32 i_morf, int param_3) {
    s16 motion = i_motion;
...
BOOL daAlbwShadeWatcher_c::drawDbgInfo() {
    return FALSE;
}

void daAlbwShadeWatcher_c::drawOtherMdls() {
    /* empty function */
}

u8 daAlbwShadeWatcher_c::getTypeFromParam() {
...
int daAlbwShadeWatcher_c::getModeFromParam() {
...
BOOL daAlbwShadeWatcher_c::isDelete() {
    // ============================================
    // NEW CODE — ALBW Port — "Shade's Refuge"
    // The Shade Watcher is a fixed rest point, not a Hero's-Shade quest
    // instance — it must always remain once spawned. All spawn gating lives
    // in d_s_room.cpp (stage/room/boomerang-bit), so never self-delete on the
    // questline appear/howl flags the way the golden wolf does.
    // ============================================
    return FALSE;
}

void daAlbwShadeWatcher_c::reset() {
    initialize();
    mpMatAnm->initialize();
    mLookat.initialize();
...
void daAlbwShadeWatcher_c::playMotion() {
...
BOOL daAlbwShadeWatcher_c::chkAction(actionFunc action) {
    return mAction == action;
}

BOOL daAlbwShadeWatcher_c::setAction(actionFunc action) {
    mMode = 3;
...
BOOL daAlbwShadeWatcher_c::selectAction() {
    mNextAction = NULL;

    if (mpHIO->m.common.debug_mode_ON) {
        mNextAction = &daAlbwShadeWatcher_c::test;
    } else {
        switch (mType) {
            case 2:
                mNextAction = &daAlbwShadeWatcher_c::waitBrave;
                break;
                  
            case 1:
                mNextAction = &daAlbwShadeWatcher_c::waitHowling;
                break;

            // ============================================
            // NEW CODE — ALBW Port — "Shade's Refuge"
            // Passive rest point: use waitBrave (holds WAITSIT, orders no
            // events) instead of the golden wolf's `wait`, which lunges at
            // Link and fires EVT_ATTACK_WARP (the Hero's-Shade attack-warp).
            // ============================================
            default:
                mNextAction = &daAlbwShadeWatcher_c::waitBrave;
                break;
...
void daAlbwShadeWatcher_c::doNormalAction(int param_1) {
...
BOOL daAlbwShadeWatcher_c::doEvent() {
    BOOL rv = FALSE;
    ...
    if (dComIfGp_event_runCheck()) {
        ...
        if (eventInfo.checkCommandTalk()) {
            if (chkAction(&daAlbwShadeWatcher_c::talk)) {
                (this->*mAction)(NULL);
            } else if (dComIfGp_event_chkTalkXY() == FALSE || dComIfGp_evmng_ChkPresentEnd()) {
                setAction(&daAlbwShadeWatcher_c::talk);
            }
...
void daAlbwShadeWatcher_c::setLookMode(int i_lookMode) {
    if (i_lookMode >= 0 && i_lookMode < 5 && i_lookMode != mLookMode) {
        mLookMode = i_lookMode;
    }
}

void daAlbwShadeWatcher_c::lookat() {
...
BOOL daAlbwShadeWatcher_c::ECut_attackWarp(int i_staffId) {
    dEvent_manager_c& eventManager = dComIfGp_getEventManager();
    int iVar1 = 0;
...
BOOL daAlbwShadeWatcher_c::ECut_attackWarpHorse(int i_staffId) {
    dEvent_manager_c& eventManager = dComIfGp_getEventManager();
    int iVar1 = 0;
    daPy_py_c* player = daPy_getPlayerActorClass();
    BOOL rv = FALSE;
...
BOOL daAlbwShadeWatcher_c::ECut_howlingSessionA(int i_staffId) {
    dEvent_manager_c& eventManager = dComIfGp_getEventManager();
...
static BOOL callback_proc(void* i_actor, int i_data) {
    static_cast<daAlbwShadeWatcher_c*>(i_actor)->meetGWolfSkip();
    return TRUE;
}

void daAlbwShadeWatcher_c::meetGWolfSkip() {
    mOrderEvtNo = EVT_NONE;
    mEventIdx = -1;
    field_0xe1d = 1;
    dComIfGp_event_reset();
...
BOOL daAlbwShadeWatcher_c::ECut_meetGWolf(int i_staffId) {
    dEvent_manager_c& eventManager = dComIfGp_getEventManager();
    daPy_getPlayerActorClass();
    int prm = -1;
    int* piVar1 = dComIfGp_evmng_getMyIntegerP(i_staffId, "prm");
...
BOOL daAlbwShadeWatcher_c::wait(void* param_1) {
    switch (mMode) {
        case 0:
            setMotion(MOT_WAITSIT, -1.0f, 0);
            setLookMode(LOOK_NONE);
...
BOOL daAlbwShadeWatcher_c::waitHowling(void* param_1) {
    switch (mMode) {
        case 0:
            setMotion(MOT_WAITSIT, -1.0f, 0);
            setLookMode(LOOK_PLAYER);
...
BOOL daAlbwShadeWatcher_c::waitBrave(void* param_1) {
    switch (mMode) {
        case 0:
            setMotion(MOT_WAITSIT, -1.0f, 0);
            setLookMode(LOOK_PLAYER);
...
BOOL daAlbwShadeWatcher_c::talk(void* param_1) {
    BOOL rv = FALSE;
    static dMsgFlow_c sShadeFlow;  // native message flow for the Rest/Home talk
    ...
    switch (mMode) {
        case 0: {
            ...
            // ============================================
            // NEW CODE — ALBW Port — "Shade's Refuge"
            // Native vanilla message box + native 2-way cursor selector, with
            // our OWN text injected at runtime (no BMG asset). initWord uses the
            // engine's 0x1324 code-text flow; setSelectWord + setSelectWordFlag
            // drive the Rest/Home select (the flag value == the choice count,
            // see d_msg_class.cpp setSelectNum). Mirrors d_a_npc_du.cpp.
            // The select words/flag MUST be set AFTER initWord (it clears them).
            // ============================================
            sShadeFlow.initWord(this,
                                "The Shade's watcher greets you, offering a\n"
                                "momentary reprieve on your travels.",
                                0xFF, 0, NULL);
            dMsgObject_setSelectWord(0, "Rest");
            dMsgObject_setSelectWord(1, "Ordon");
        ...
        case 2:
            if (sShadeFlow.doFlow(this, NULL, 0)) {
                // Capture the choice, then CLEAR the native 2-way select state.
                // initWord leaves setSelectWordFlag(2) + the "Rest"/"Ordon"
                // labels set, and the engine only resets them on the NEXT message
                // flow's init. The game-over save menu (Retry Yes/No) is ALSO a
                // message select, so after a rest it inherited our stale select
                // state and failed to render — stalling the whole game-over (no
                // Retry, hence no warp toast). Clearing it here fixes that.
                const int choice = dMsgObject_getSelectCursorPos();  // 0 = Rest, 1 = Ordon
                // Clearing the select-word COUNT is what unbreaks the next
                // message (the save menu reads getSelectWordFlag(); a stale 2
                // made it inherit our select). The labels/word are only read
                // when the flag is non-zero, so leave those buffers alone —
                // writing "" into the global message object mid-talk and then
                // warping (Ordon) corrupted state and crashed a few steps later.
                dMsgObject_setSelectWordFlag(0);

                // Both Rest and Ordon "check in" at THIS watcher: register it as
                // the single active respawn/return slot (overwrites the previous
                // watcher) so the death toast AND the shop's "Return to Last
                // Shade Watcher" both point here. resolveLinkSpawn picks this
                // watcher's custom Link spawn point if one is set, else the
                // wolf's own spot — so Link can land beside/facing the wolf.
                cXyz spawnPos;
                s16  spawnAngle;
                dShadeRefuge_resolveLinkSpawn(dComIfGp_getStartStageName(),
                                              fopAcM_GetRoomNo(this), home.pos,
                                              home.angle.y, &spawnPos, &spawnAngle);
                dShadeRefuge_setRespawn(dComIfGp_getStartStageName(),
                                        fopAcM_GetRoomNo(this), spawnPos, spawnAngle);

                if (choice == 1) {
                    // Ordon: warp to Ordon Village.
                    dComIfGp_setNextStage("F_SP103", 0, 1, -1);
                } else {
                    // Rest: full heal via the recovery path (caps at the TRUE
                    // max incl. Master-Quest bonus half-hearts + plays the fairy
                    // sound; setLife(getMaxLife()) misses the bonus) + ALBW
                    // stamina.
...
BOOL daAlbwShadeWatcher_c::test(void* param_1) {
    switch (mMode) {
        case 0:
            speedF = 0.0f;
            speed.setall(0.0f);
...
s16 daAlbwShadeWatcher_c::getGroundSlope(dBgS_ObjAcch& i_acch, s16 param_2) {
    cM3dGPla plane;
...
static int daAlbwShadeWatcher_Create(void* a_this) {
    return static_cast<daAlbwShadeWatcher_c*>(a_this)->create();
}

static int daAlbwShadeWatcher_Delete(void* a_this) {
    return static_cast<daAlbwShadeWatcher_c*>(a_this)->Delete();
}

static int daAlbwShadeWatcher_Execute(void* a_this) {
    return static_cast<daAlbwShadeWatcher_c*>(a_this)->Execute();
}

static int daAlbwShadeWatcher_Draw(void* a_this) {
    return static_cast<daAlbwShadeWatcher_c*>(a_this)->Draw();
}

static int daAlbwShadeWatcher_IsDelete(void* a_this) {
    return 1;
}

AUDIO_INSTANCES;

static DUSK_CONST actor_method_class daAlbwShadeWatcher_MethodTable = {
    (process_method_func)daAlbwShadeWatcher_Create,
    (process_method_func)daAlbwShadeWatcher_Delete,
    (process_method_func)daAlbwShadeWatcher_Execute,
    (process_method_func)daAlbwShadeWatcher_IsDelete,
    (process_method_func)daAlbwShadeWatcher_Draw,
};

DUSK_PROFILE actor_process_profile_definition DUSK_CONST g_profile_ALBW_SHADE_WATCHER = {
    /* Layer ID     */ fpcLy_CURRENT_e,
    /* List ID      */ 7,
    /* List Prio    */ fpcPi_CURRENT_e,
    /* Proc Name    */ fpcNm_ALBW_SHADE_WATCHER_e,
    /* Proc SubMtd  */ &g_fpcLf_Method.base,
    /* Size         */ sizeof(daAlbwShadeWatcher_c),
    /* Size Other   */ 0,
    /* Parameters   */ 0,
    /* Leaf SubMtd  */ &g_fopAc_Method.base,
    /* Draw Prio    */ fpcDwPi_ALBW_SHADE_WATCHER_e,
    /* Actor SubMtd */ &daAlbwShadeWatcher_MethodTable,
