 ...
 *
 * ============================================================
 * §254 DIRECT-PORT RECIPES APPLIED (each marked §254 at its site):
 *   cPhs_State->cPhs_Step; ARRAY_SSIZE->ARRAY_SIZE; (u32)this->(uintptr_t)this;
 *   fopAcM_ct_Retail->fopAcM_ct; VERSION_SELECT collapses to the US/retail arg;
 *   dComIfG_Bgsp() reference (-> dropped); tevStr mRoomNo->room_no /
 *   mEnvrIdxOverride->YukaCol; msg_class mStatus->mode / mSelectNum->select_idx;
 *   fopMsgStts_*->fopMsg_MODE_*; dSv_event_flag_c::UNK_*->WWEV_UNK_* (Aryll §244);
 *   DN-3 (body + cloth models via dExtNpcMount_acquireModelData + NULL-guard->FALSE);
 *   render recipe #2/#13 (modelCalc()->face btp/btk entry->setList->entryDL);
 *   node-CB J3DNode*->J3DJoint*; LIGHT_INFLUENCE mPower->mPow / mPos->mPosition;
 *   port API arities (evmng_getEventIdx +0xFF; orderSpeakEvent +2; orderOtherEventId
 *   +4; evmng_getMyStaffId +this+0; dDemo_setDemoData 8-arg; btp/btk init 9->7).
 *
 * §254 DEFERRED-FIDELITY BRIDGES (donor-faithful in effect; flagged for History):
 *   1. WW eye-over-hair cup/aup layered _draw (mDoExt_*CupAup*Packet::entryOpa +
 *      setListP0/P1 + per-shape show/hide + McaMorf::entry) — the port's J3DPacket
 *      has no entryOpa; the eyes still render as model joints via the morf entryDL,
 *      the punch-through-hair blend is deferred. The create-time material/shape
 *      cache walk (mJoint2/3->getMesh()) is omitted with it.
 *   2. daShip_c (KoRL) + daBranch_c (tree) mount partners — port-absent; shimmed so
 *      the swoon/branch-hang blocks compile and stay runtime-inert (no such actor
 *      spawns). Real props owed for the departure/rescue + Outset-awake cutscenes.
 *   3. Omamori-glow / simple-land splash FX + evmng_cancelStartDemo — inert shims.
 *
 * §254 REGISTRATION = REPORT-ONLY (History follow-up): fpcNm_NPC_ZL1_e is still the
 *   d_ext_ww_actor_shims.h sentinel (0xFFFB) for this green build; the real slot
 *   0x329 (+ d_stage "Zl1" demo-binding row + removing the sentinel) is reported,
 *   not applied. files.cmake TU entry is a TEMP build-verify edit, reverted.
 * ============================================================
 */
...
#include "d/d_ext_ww_actor_shims.h"     // §254 daShip_c/daBranch_c + dSnap_RegistFig + WWEV_/shims
#include "d/d_bg_s.h"                   // §254 dBgS_GetGndMtrlSndId_Func + GetTriPla (bm1 §246 precedent)
#include "d/d_bg_s_roof_chk.h"
#include "d/d_bg_s_lin_chk.h"
#include "m_Do/m_Do_ext.h"
#include "SSystem/SComponent/c_counter.h"
#include "res/Object/Zl.h"
#include "d/d_ext_npc_mount.h"          // §254 DN-3 parse-at-consume (acquireModelData)
...
//   JUT_ASSERT(VERSION_SELECT(...), cond) lines verbatim.
#ifndef VERSION_SELECT
#define VERSION_SELECT(a_demo, a_us, a_pal, a_jp) (a_us)
#endif

// §254 WW fopAcM_searchPlayerDistance2 (squared player distance) — the port ships
//   only the linear fopAcM_searchPlayerDistance; Tetra's optn paths compare against
//   temp*temp, so square the linear result. TU-local (no name-space collision).
static f32 fopAcM_searchPlayerDistance2(const fopAc_ac_c* actor) {
    f32 d = fopAcM_searchPlayerDistance(actor);
    return d * d;
}
...
daNpc_Zl1_HIO_c::daNpc_Zl1_HIO_c() {
    static hio_prm_c a_prm_tbl = {
        /* mMaxHeadX     */ 0x18E2,
        /* mMaxHeadY     */ 0x2328,
        /* mMinHeadX     */ (s16)0xE71E,
        /* mMinHeadY     */ (s16)0xDCD8,
        /* mMaxBackboneX */ 0x0BB8,
        /* mMaxBackboneY */ 0x03E8,
        /* mMinBackboneX */ (s16)0xF8E4,
        /* mMinBackboneY */ (s16)0xFC18,
        /* mMaxTurnStep  */ 0x1000,
        /* field_18      */ 0x0800,
        /* field_1C      */ 140.0f,
        /* field_20      */ 0,
...
void daNpc_Zl1_matAnm_c::calc(J3DMaterial* mat) const {
    ...
    // §254 port: getTexMtxAnm(i) has no port equivalent; the WW "slot active?"
    //   test maps to getTexMtx(i) != NULL (the write target). getTextureSRT() ->
    //   getTexMtxInfo().mSRT (port rename) — mirrors Aryll's matAnm calc.
    for (u32 i = 0; i < 8; i++) {
        J3DTexMtx* tex_mtx_p = mat->getTexMtx(i);
        if (tex_mtx_p != NULL) {
            if (field_0x7C) {
                tex_mtx_p->getTexMtxInfo().mSRT.mTranslationX = mOffset.x;
                tex_mtx_p->getTexMtxInfo().mSRT.mTranslationY = mOffset.y;
            }
        }
    }
}
...
static BOOL nodeCB_Head(J3DJoint* i_node, int i_param) {
...
void daNpc_Zl1_c::_nodeCB_Head(J3DJoint* i_node, J3DModel* i_pModel) {
...
static BOOL nodeCB_BackBone(J3DJoint* i_node, int i_param) {
    if (i_param == J3DNodeCBCalcTiming_In) {
        daNpc_Zl1_c* actor = (daNpc_Zl1_c*)j3dSys.getModel()->getUserArea();
        if (actor != NULL) {
...
void daNpc_Zl1_c::_nodeCB_BackBone(J3DJoint* i_node, J3DModel* i_pModel) {
    int jointIdx = ((J3DJoint*)(i_node))->getJntNo();
    mDoMtx_stack_c::copy(i_pModel->getAnmMtx(jointIdx));
...
static void* searchActor_Branch(void* i_param1, void*) {
    if (l_check_wrk < ARRAY_SIZE(l_check_inf) && fopAcM_IsActor(i_param1) && fopAcM_GetName(i_param1) == fpcNm_BRANCH_e) {
...
static void* searchActor_Bm1(void* i_param1, void*) {
    if (l_check_wrk < ARRAY_SIZE(l_check_inf) && fopAcM_IsActor(i_param1) && fopAcM_GetName(i_param1) == fpcNm_NPC_BM1_e) {
        l_check_inf[l_check_wrk] = (fopAc_ac_c*)i_param1;
        l_check_wrk++;
    }
    return NULL;
...
BOOL daNpc_Zl1_c::set_startPos(int param_1) {
    ...
    if (param_1 == 0 || param_1 == 1) {
        ...
        if (temp2 < 3) {
            ...
            *mObjAcch.GetOldPos() = current.pos;  // §254 port dBgS_ObjAcch has no SetOld()
        }
    }
    return TRUE;
}
...
bool daNpc_Zl1_c::init_ZL1_0() {
    if (!dComIfGs_isEventBit(WWEV_UNK_0001)) {
...
bool daNpc_Zl1_c::init_ZL1_1() {
    if (!dComIfGs_isEventBit(WWEV_UNK_0101)) {
        set_action(&daNpc_Zl1_c::demo_action1, NULL);
...
bool daNpc_Zl1_c::init_ZL1_2() {
    if (dComIfGs_isEventBit(WWEV_UNK_0E20) && !dComIfGs_isEventBit(WWEV_UNK_2401)) {
...
bool daNpc_Zl1_c::init_ZL1_3() {
    if (!dComIfGs_isEventBit(WWEV_UNK_0801)) {
...
bool daNpc_Zl1_c::init_ZL1_5() {
    if (!dComIfGs_isEventBit(WWEV_UNK_2D02)) {
        set_action(&daNpc_Zl1_c::optn_action1, NULL);
        fopAcM_OffStatus(this, fopAcStts_CULL_e | fopAcStts_NOCULLEXEC_e);
#if VERSION > VERSION_DEMO
        if (!dComIfGs_isEventBit(WWEV_UNK_3804)) {
            fopAcM_OnStatus(this, fopAcStts_UNK4000_e);
            field_0x84A = 6;
            mEventIdx[4] = field_0x84A + -3;
            // §254 port arity (+mapToolID,+param3,+param4,+param5) — matches Aryll §244
            fopAcM_orderOtherEventId(this, mEventIdx[mEventIdx[4]], 0xFF, 0xFFFF, 4, 1);
...
bool daNpc_Zl1_c::createInit() {
    ...
    for (int i = 0; i < ARRAY_SIZE(l_evn_tbl); i++) {
        mEventIdx[i] = dComIfGp_evmng_getEventIdx(l_evn_tbl[i], 0xFF);  // §254 port arity (+mapToolID)
    }
    mEventCut.setActorInfo2((char*)"Zl1", this);  // §254 port setActorInfo2 takes char*
    ...
    mObjAcch.CrrPos(dComIfG_Bgsp());  // §254 Bgsp() reference (donor *deref drop)
    play_animation();
...
bool daNpc_Zl1_c::swoon_OnShip() {
    // §254 daShip_c (KoRL) is a deferred bridge: dComIfGp_getShipActor() shims to
    //   NULL, so this returns false (the ship-relative transform is never applied).
        ...
        if (!fpcM_IsCreating(mProcId1)) {
            if (mProcId1 != fpcM_ERROR_PROCESS_ID_e) {
                ...
                if (fopAcM_SearchByID(mProcId1, (fopAc_ac_c**)&actor) == 1) {
                    if (actor != NULL) {
                        // §254 daBranch_c is a deferred bridge: fpcNm_BRANCH_e is a
                        //   sentinel that never matches, so mProcId1 stays ERROR and
                        //   this block is never entered (getJointMtx shims to NULL).
...
#else
        // §254 the port build is VERSION==VERSION_DEMO (-DVERSION=0), so this
        //   (demo) branch is the LIVE one. daBranch_c is a deferred bridge:
        //   mProcId1 stays ERROR (fpcNm_BRANCH_e never matches) -> inert.
        BOOL wasDeleted;
...
bool daNpc_Zl1_c::setBtp(s8 param_1, bool param_2) {
    ...
    mBtpAnmFrame = 0;
    mTimer = 0;
    // §254 btp/btk init 9->7 args (drop trailing modify flag + pad) — Aryll §244.
    return mBtpAnm.init(model->getModelData(), a_btp, TRUE, J3DFrameCtrl::EMode_NONE, 1.0f, 0, -1) != 0;
}
...
bool daNpc_Zl1_c::setBtk(s8 param_1, bool param_2) {
    ...
    mBtkAnmFrame = 0;
    // §254 btp/btk init 9->7 args (drop trailing modify flag + pad) — Aryll §244.
    if (mBtkAnm.init(model->getModelData(), a_btk, TRUE, J3DFrameCtrl::EMode_NONE, 1.0f, 0, -1) != 0) {
        if (!param_2) {
            setMat();
        }
        return true;
...
u16 daNpc_Zl1_c::next_msgStatus(u32* pMsgNo) {
    u16 msg_status = fopMsg_MODE_MSG_CONTINUE_e;
    ...
    switch (*pMsgNo) {
        ...
        case 0xCA3:
            *pMsgNo = 0xC90;
            break;
        case 0xC90: {
            switch (mpCurrMsg->select_idx) {
                case 0:
                    msg_status = fopMsg_MODE_MSG_END_e;
                    break;
                case 1:
                    *pMsgNo = 0xC91;
                    break;
...
u32 daNpc_Zl1_c::getMsg_ZL1_2() {
    u32 temp;
    // §254 dComIfGs_checkCollect -> Aryll's dComIfGs_getpCollect()->checkCollect shim.
    if (dComIfGs_getpCollect()->checkCollect(1) != 0) {
        temp = 0xCA2 + (dComIfGs_isEventBit(WWEV_UNK_2908) != 0 ? 1 : 0);
    } else {
        temp = 0xCA1;
    }
    return temp;
...
u32 daNpc_Zl1_c::getMsg_ZL1_4() {
    return 0x1005 + (dComIfGs_isEventBit(WWEV_UNK_0810) != 0 ? 1 : 0);
}
...
void daNpc_Zl1_c::eventOrder() {
    if (field_0x84A == 1 || field_0x84A == 2) {
        ...
        if (field_0x84A == 1) {
            fopAcM_orderSpeakEvent(this, 0, 0);  // §254 port arity (+priority,+flag)
        }
    } else if (field_0x84A >= 3) {
        mEventIdx[4] = field_0x84A - 3;
        // §254 port arity (+mapToolID,+param3,+param4,+param5) — Aryll §244
        fopAcM_orderOtherEventId(this, mEventIdx[mEventIdx[4]], 0xFF, 0xFFFF, 4, 1);
    }
}
...
void daNpc_Zl1_c::checkOrder() {
        ...
        if (dComIfGp_evmng_startCheck(mEventIdx[mEventIdx[4]]) && field_0x84A >= 3) {
            switch (mEventIdx[4]) {
                case 0:
                    ...
                    break;
                case 2:
                    dComIfGs_setRestartRoom(*(cXyz*)a_start_pos, 0x4000, 0);
                    field_0x7C9 = true;
                    break;
                case 3:
    ...
    } else if (eventInfo.checkCommandTalk() && (field_0x84A == 1 || field_0x84A == 2)) {  // §254 getCommand()==INTALK -> checkCommandTalk() (Aryll §244)
        field_0x84A = 0;
        field_0x7D7 = true;
    }
}
...
bool daNpc_Zl1_c::partner_search_sub(fpcLyIt_JudgeFunc i_judgeFunc) {
    ...
    l_check_wrk = 0;
    for (s32 i = 0; i < ARRAY_SIZE(l_check_inf); i++) {
        l_check_inf[i] = NULL;
    }
...
void daNpc_Zl1_c::init_Light() {
    // §254 LIGHT_INFLUENCE port field names: mPower->mPow, mPos->mPosition.
    mLightInfluence1.mColor.r = 0x5c;
    mLightInfluence1.mColor.g = 0xae;
    mLightInfluence1.mColor.b = 0xff;
    mLightInfluence1.mPow = 139.7f;
...
void daNpc_Zl1_c::cut_init_OMAMORI_ONOFF(int i_staffIdx) {
    ...
    if (pOnOff != NULL) {
        if (*pOnOff == 0) {
#if VERSION > VERSION_DEMO
            mDoAud_seStart(JA_SE_ITM_OMAMORI_TETLA, NULL, 0, 0);  // §254 port seStart 4-arg
#endif
            field_0x7C2 = -1;
            mRatio = 1.0f;
            init_Light();
            field_0x830 = set_LightPos();
            mLightInfluence1.mPosition = field_0x830;  // §254 mPos->mPosition
            mLightInfluence2.mPosition = field_0x830;
        } else {
            field_0x7C2 = 1;
            mRatio = 0.15f;
            mLightInfluence2.mPow = 0.0f;  // §254 mPower->mPow
            mLightInfluence1.mPow = 0.0f;
        }
    }
}
...
void daNpc_Zl1_c::cut_init_SURPRISED(int) {
    mDoAud_seStart(JA_SE_ITM_OMAMORI_BLINK, NULL, 0, 0);  // §254 port seStart 4-arg
...
void daNpc_Zl1_c::privateCut(int i_staffIdx) {
    static char* a_cut_tbl[] = {
        ...
        (char*)"LOK_PARTNER",
        (char*)"CHG_ANM_ATR",
        (char*)"PLYER_TRN_PARTNER",
        (char*)"PLYER_TRN_TETRA",
        (char*)"MAJYU_START",
        (char*)"MAJYU_END",
        (char*)"OKIRU",
        (char*)"OKIRU_2",
        (char*)"DRW_ONOFF",
        (char*)"PLYER_DRW_ONOFF",
        (char*)"JMP_OFF",
        (char*)"OMAMORI_ONOFF",
#if VERSION > VERSION_DEMO
        (char*)"SURPRISED",
...
int daNpc_Zl1_c::isEventEntry() {
    return dComIfGp_evmng_getMyStaffId(mEventCut.getActorName(), this, 0);  // §254 port arity
}
...
void daNpc_Zl1_c::event_proc(int i_staffIdx) {
    if (dComIfGp_evmng_endCheck(mEventIdx[mEventIdx[4]])) {
        switch (mEventIdx[4]) {
            case 0:
                field_0x7D3 = false;
                dComIfGs_onEventBit(WWEV_UNK_0802);
                field_0x72C = current.pos;
                field_0x738 = current.angle;
                setStt(1);
                break;
            case 1:
                dComIfGs_onEventBit(WWEV_UNK_0804);
                ...
                break;
            case 2:
                dComIfGs_onEventBit(WWEV_UNK_0801);
                field_0x7C9 = false;
                break;
            case 3:
                dComIfGs_onEventBit(WWEV_UNK_3804);
                setStt(3);
                field_0x7CB = false;
                break;
        }
    ...
    if (!field_0x7CA) {
        if (mObjAcch.ChkWaterIn()) {
            if (mRippleCallBack.getEmitter() == NULL) {
                dComIfGp_particle_setShipTail(ID_AK_JN_HAMON00, &current.pos, NULL, NULL, 0xff, &mRippleCallBack);  // §254 unscoped enum
            }
...
void daNpc_Zl1_c::setEff() {
    if (field_0x849 == 0xb || field_0x7C6) {
        if (mObjAcch.ChkWaterIn()) {
            ...
            // §254 dBgS::GetAttributeCode absent -> dExtKb_GetAttributeCode shim
            //   (returns a constant != 0x13, so this splash branch is inert-faithful).
            int attrCode = dExtKb_GetAttributeCode(mObjAcch.m_gnd);
...
BOOL daNpc_Zl1_c::setFrontWallType() {
    ...
    // §254 port GetTriPla(polyInfo, out*)->bool (donor returned the plane*). The
    //   line-check (dBgS_LinChk) converts to cBgS_PolyInfo (see d_a_alink GetTriPla).
    cM3dGPla plane_storage;
    dComIfG_Bgsp().GetTriPla(linChk, &plane_storage);
    cM3dGPla* plane = &plane_storage;
...
BOOL daNpc_Zl1_c::talk_1() {
    ...
    if (field_0x7D7) {
        ...
        switch (mpCurrMsg->mode) {
            case fopMsg_MODE_BOX_OPENING_e:
                break;
            case fopMsg_MODE_MSG_TYPING_e:
                break;
            case fopMsg_MODE_MSG_DESTROYED_e:
                switch (mCurrMsgNo) {
                    case 0xC91:
                        dComIfGs_onEventBit(WWEV_UNK_2908);
                        break;
                    case 0x1005:
                        dComIfGs_onEventBit(WWEV_UNK_0810);
                        break;
                    case 0xC90:
                        // §254 dComIfGp_setNextStage: WW 8-arg -> port 4-arg overload
                        //   (name, point, room, layer); the WW wipe/fade tail args
                        //   are dropped (cosmetic). Outset -> sea departure warp.
                        dComIfGp_setNextStage("sea", 0xcd, dIsleRoom_OutsetIsland_e, 10);
                        field_0x7D7 = false;
                        return ret;
                }
                mItemNo = 0xFF;
...
BOOL daNpc_Zl1_c::demo_1() {
    if (field_0x84F == 0 && dComIfGs_isEventBit(WWEV_UNK_0001)) {
        fopAcM_delete(this);
    }
    return TRUE;
}
...
int daNpc_Zl1_c::wait_action1(void*) {
    switch (field_0x850) {
        case 0:
            if (field_0x84F == 2 && !dComIfGs_isEventBit(WWEV_UNK_0802)) {
                setStt(9);
                field_0x850++;
            } else {
                setStt(1);
...
int daNpc_Zl1_c::demo_action2(void*) {
    switch (field_0x850) {
        case 0:
            if (dComIfGs_isEventBit(WWEV_UNK_0804)) {
                setStt(8);
                current.pos.set(-37692.0f, 2200.0f, 8016.0f);
                current.angle.y = -0x5C72;
                field_0x764.y = current.pos.y;
...
u8 daNpc_Zl1_c::demo() {
    if (demoActorID == 0) {
        ...
        // §254 recipe 11: dComIfGp_demo_getActor returns the real actor (shared
        //   shim, dDemo_c::getActor). §252: getP_BtpData/BtkData are native.
        ...
        if (btp != NULL) {
            mBtpAnm.init(mpMorf->getModel()->getModelData(), btp, TRUE, J3DFrameCtrl::EMode_NONE, 1.0f, 0, -1);
            field_0x847 = 0x11;
            mBtpAnmFrame = 0;
        }
        ...
        if (btk != NULL) {
            mBtkAnm.init(mpMorf->getModel()->getModelData(), btk, TRUE, J3DFrameCtrl::EMode_NONE, 1.0f, 0, -1);
            field_0x848 = 10;
            mBtkAnmFrame = 0;
        }

        if (field_0x7CE) {
            cXyz temp = demo_actor->getTrans();  // §254 port getTrans() returns cXyz& (donor returned a ptr)
            if ((int)temp.x != 0 || (int)temp.y != 0 || (int)temp.z != 0) {
                mpModel = NULL;
                field_0x7CE = false;
            }
        }
        // §254 dDemo_setDemoData: port is always 8-arg (Aryll §244). The build is
        //   VERSION==VERSION_DEMO (-DVERSION=0), and dBgS_GetGndMtrlSndId_Func only
        //   exists in the port's dead retail #if branches, so the live demo path
        //   passes 0/0 for snd/reverb (matches Aryll's demo branch exactly).
        u32 enableFlags = dDemo_actor_c::ENABLE_TRANS_e | dDemo_actor_c::ENABLE_ROTATE_e | dDemo_actor_c::ENABLE_ANM_e | dDemo_actor_c::ENABLE_ANM_FRAME_e;
#if VERSION > VERSION_DEMO
        dDemo_setDemoData(this, enableFlags, mpMorf, mArcName, 0, NULL,
            dBgS_GetGndMtrlSndId_Func(current.pos, 10.0f),
            dComIfGp_getReverb(fopAcM_GetRoomNo(this)));
#else
        dDemo_setDemoData(this, enableFlags, mpMorf, mArcName, 0, NULL, 0, 0);
#endif
    }
    return field_0x7D9;
}
...
void daNpc_Zl1_c::shadowDraw() {
    ...
    mShadowId = dComIfGd_setShadow(
        mShadowId,
        0,  // §254 retail (VERSION > VERSION_JPN) shadow type
        mpMorf->getModel(),
        &pos, 800.0f, 40.0f, current.pos.y,
        mObjAcch.GetGroundH(), mObjAcch.m_gnd,
        &tevStr,
        // §254 port setShadow adds 3 trailing shadow-tex params (Aryll §244).
...
BOOL daNpc_Zl1_c::_draw() {
    ...
    //   The WW eye-over-hair cup/aup layered multi-pass (mDoExt_*CupAup*Packet
    //   entryOpa + setListP0/P1 + per-shape show/hide + McaMorf::entry) is a
    //   WW-packet API the port's J3DPacket lacks (no entryOpa) -> the eyes still
    //   render as model joints via the morf entryDL; the punch-through-hair blend
    //   is a DEFERRED fidelity bridge (see file header).
    mpMorf->modelCalc();
    mBtpAnm.entry(modelData, mBtpAnmFrame);
    mBtkAnm.entry(modelData, (f32)mBtkAnmFrame);
    dComIfGd_setList();
    mpMorf->entryDL();
...
BOOL daNpc_Zl1_c::_delete() {
    // §254 port has no dComIfG_resDeleteDemo -> dComIfG_resDelete (Aryll §244).
    dComIfG_resDelete(&mPhs, mArcName);
    if (heap != NULL && mpMorf != NULL) {
        // §254 port McaMorf lacks stopZelAnime -> shared shim (bm1 §246).
...
cPhs_Step daNpc_Zl1_c::_create() {
    ...
    cPhs_Step state;
    fopAcM_ct(this, daNpc_Zl1_c);  // §254 WW fopAcM_ct_Retail -> port fopAcM_ct
...
BOOL daNpc_Zl1_c::bodyCreateHeap() {
    // §254 DN-3 parse-at-consume (recipe 1): the McaMorf ctor derefs
    //   getMaterialNodePointer(0); a raw getObjectRes buffer -> AV. Route the body
    //   model through dExtNpcMount_acquireModelData. NULL-guard -> FALSE.
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "zl.bdl");
    if (a_mdl_dat == NULL) {
        return FALSE;
    }
    ...
    // §254 DEFERRED (see file header): the donor here walks mJoint2/3->getMesh()->
    //   getNext() to cache the eye/eyebrow J3DMaterial*/J3DShape* (field_0x860/878/
    //   890/8A8) for the WW cup/aup layered _draw. That layered pass has no port
    //   entryOpa equivalent, so both it and this create-time cache walk are omitted
    //   (nothing reads those fields); the eyes render as ordinary model joints.
    ...
    mpMorf->getModel()->setUserArea((uintptr_t)(this));  // §254 (u32)this -> (uintptr_t)this
    return TRUE;
}
...
BOOL daNpc_Zl1_c::itemCreateHeap() {
    ...
    // §254 DN-3 parse-at-consume for the cloth model (own Zl arc). NULL-guard.
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "cloth.bdl");
    if (a_mdl_dat == NULL) {
        return FALSE;
    }
...
BOOL daNpc_Zl1_c::CreateHeap() {
    ...
    mLightInfluence1.mPow = 0.0f;  // §254 mPower->mPow
    mLightInfluence2.mPow = 0.0f;
    dKy_plight_priority_set(&mLightInfluence1);
    dKy_efplight_set(&mLightInfluence2);

    mAcchCir.SetWall(30.0f, 50.0f);
    // §254 port dBgS_ObjAcch::Set 8-arg (Aryll §244: +2 trailing NULL).
...
static cPhs_Step daNpc_Zl1_Create(fopAc_ac_c* i_this) {
