...
#include "f_op/f_op_camera_mng.h"           // §226 camera_process_class full type (mCamera/ForceLockOff)
#include "JSystem/J3DGraphAnimator/J3DJoint.h" // §226 J3DJoint node callback (nodeCallBack)
...
void tex_anm_set(kb_class* i_this, u16 param_1) {
    ...
    i_this->m50A = 2;
    i_this->m508 = 0;
    // §229 DN-3: m500 (BTP eye pattern) is a RAW resource — setFrame/getUpdateMaterialNum
    // would deref raw bytes. Eye tex-anm application is a cosmetic no-op; NULL the pattern
    // (the m500->setFrame call sites are guarded) and skip the raw bind.
    i_this->m500 = NULL;
}
...
BOOL carry_check(kb_class* i_this) {
    ...
    if(fopAcM_checkCarryNow(actor)) {
        ...
        cLib_offBit<u32>(actor->attention_info.flags, fopAc_AttnFlag_LOCK_e);
...
void speed_pos_set(kb_class* i_this) {
    ...
    if(dExtKb_getGrabMissActor() == actor) {
        i_this->m40B = 1;
...
void hamon_set(kb_class* i_this) {
    ...
    dComIfGp_particle_setShipTail(ID_AK_JN_HAMON00, &actor->current.pos, NULL, &scale, 0xFF, &i_this->m540);
}
#else
...
void sibuki_set(kb_class* i_this) {
    ...
    i_this->m4C4 = 30.0f;
    if(i_this->m4D4) {
        scaleXZ = BREG_F(4) + 0.8f;
    }
    fopKyM_createWpillar(&pos, scaleXZ, 0);
...
void pl_attack_hit_check(kb_class* i_this) {
    ...
    if(i_this->mSph.ChkTgHit()) {
        if(i_this->m403 == 0) {
            ...
            dComIfGp_particle_set(ID_AK_JN_TUBA00, &actor->current.pos, &actor->shape_angle, NULL);
...
void he_set(kb_class* i_this) {
    ...
    if(i_this->m594.getEmitter() == NULL) {
        ...
        dComIfGp_particle_set(ID_AK_SN_PIGGAS00, &i_this->m5D4[0], NULL, NULL, 0xFF, &i_this->m594, fopAcM_GetRoomNo(actor), NULL, NULL, NULL);
    }
...
void smoke_set(kb_class* i_this) {
    ...
    if(i_this->m554.getEmitter() == NULL) {
        dComIfGp_particle_setToon(ID_AK_JT_ELEMENTSMOKE00, &i_this->m5D4[0], &i_this->m5EC[0], NULL, 0xB9, &i_this->m554, fopAcM_GetRoomNo(actor));
    }
...
void smoke_set2(kb_class* i_this) {
    ...
    if(i_this->m554.getEmitter() == NULL) {
        dComIfGp_particle_setToon(ID_AK_JT_ELEMENTSMOKE01, &i_this->m5D4[0], &actor->shape_angle, NULL, 0xB9, &i_this->m554, fopAcM_GetRoomNo(actor));
    }
...
void smoke_set3(kb_class* i_this) {
    ...
    if(i_this->m574.getEmitter() == NULL) {
        dComIfGp_particle_setToon(ID_AK_ST_PIGDIGSMOKE00, &i_this->m5D4[1], &actor->shape_angle, NULL, 0xA0, &i_this->m574, fopAcM_GetRoomNo(actor));
    }
...
static BOOL nodeCallBack(J3DJoint* i_node, int calcTiming) {
...
static BOOL daKb_Draw(kb_class* i_this) {
    ...
    dExtKb_setMaterialTable(pModelData, i_this->mpMaterialTable);
    i_this->mTevStr = actor->tevStr;
    i_this->mTevStr.TevColor.r = actor->tevStr.TevColor.r + (0x78 - actor->tevStr.TevColor.r) * i_this->m4B0;
    i_this->mTevStr.TevColor.g = actor->tevStr.TevColor.g + (-actor->tevStr.TevColor.g) * i_this->m4B0;
    i_this->mTevStr.TevColor.b = actor->tevStr.TevColor.b + (-actor->tevStr.TevColor.b) * i_this->m4B0;
    ...
    // so the world/joint matrices were never computed → invisible. Mirror the proven mount
    // draw path (d_ext_npc_mount.cpp:9014 modelCalc + 9398/9410 setList/entryDL).
    ...
    if(
        ...
        cXyz shadowPos(actor->current.pos.x, actor->current.pos.y + 150.0f + BREG_F(0x12), actor->current.pos.z);
        i_this->mShadowId = dComIfGd_setShadow(i_this->mShadowId, 
                                     1, 
                                     pModel, 
                                     &shadowPos, BREG_F(0x13) + 800.0f, BREG_F(0x11) + 40.0f, 
                                     actor->current.pos.y, 
                                     i_this->mAcch.GetGroundH(),
                                     i_this->mAcch.m_gnd,
                                     &actor->tevStr,
                                     // §226 port setShadow adds 3 trailing params (shadow-tex);
                                     // standard actor values, matches d_a_b_dr / alink.
        ...
        case 0xB:
            ...
            if(!fopAcM_checkCarryNow(actor)) {
                ...
                if(actor->speedF <= 0.0f) {
                    ...
                    i_this->m420 = 0xD;
                    cLib_onBit<u32>(actor->attention_info.flags, fopAc_AttnFlag_LOCK_e);
                }
                else {
                    i_this->m436 = 0;
                    i_this->m4C4 = 30.0f;
        ...
        case 0xC:
            ...
            if(i_this->mShapeType >= 8) {
                ...
                if(strcmp(dComIfGp_getStartStageName(), "sea") == 0 && fopAcM_GetRoomNo(actor) == dIsleRoom_OutsetIsland_e && i_this->mAcch.GetGroundH() != -G_CM3D_F_INF && dComIfG_Bgsp().GetSpecialCode(i_this->mAcch.m_gnd) != 4 && !dKy_daynight_check()) {
                    dComIfGs_onEventBit(dSv_event_flag_c::F_0428);
                }
            }
...
void swim_move(kb_class* i_this) {
    ...
    switch(i_this->m420) {
        ...
        case 0x16:
        {
            f32 temp3 = i_this->m4D4 * (BREG_F(6) + 10.0f) + 10.0f;
            ...
            f32 temp4 = cM_ssin(i_this->m436) * (BREG_F(7) + 5.0f) + 29.0f;
            temp4 += i_this->m4D4 * (BREG_F(8) + 29.0f);
        ...
        case 0x19:
            ...
            if(i_this->mpMorf->checkFrame(10.0f) || i_this->mpMorf->checkFrame(11.0f)) {
                ...
                if(i_this->m5A8[temp3].getEmitter() == NULL) {
                    dComIfGp_particle_set(ID_AK_SN_PIGDASSUI, &actor->current.pos, &temp, NULL, 0xFF, &i_this->m5A8[temp3], fopAcM_GetRoomNo(actor), NULL, NULL, NULL);
                }
...
void money_drop(kb_class* i_this) {
    ...
    static u32 item_rate_dt[] = {
        dItemNo_ARROW_10_e, dItemNo_BOMB_5_e, dItemNo_S_MAGIC_e
    };
    ...
    for(i = 0; i < 2; i++) {
        ...
        if(BREG_S(4) == 0) {
            if(rnd < 0.9f) {
                ...
                fopAc_ac_c* pItem = (fopAc_ac_c*)dExtKb_fastCreateItem(&temp4, temp2, -1, &temp, &actor->scale, cM_rndF(5.0f), cM_rndF(10.0f) + 20.0f, -3.0f);
                if(pItem) {
                    fopAcM_OnStatus(pItem, fopAcStts_UNK_0x4000_e);
...
void esa_demo_move(kb_class* i_this) {
    ...
    switch(i_this->m420) {
        ...
        case 0x29:
            ...
            pPlayer->changeDemoMode(daPy_demo_c::DEMO_UNK_1_e, 0, 0, 0);
        ...
        case 0x2A:
        {
            i_this->m520.x = actor->current.pos.x + NREG_F(1);
            i_this->m520.y = actor->current.pos.y + 60.0f + NREG_F(2);
            i_this->m520.z = actor->current.pos.z + NREG_F(3);
            cMtx_YrotS(*calc_mtx, NREG_F(4));
            cXyz temp;
            temp.x = 0.0f;
            temp.y = 0.0f;
            temp.z = NREG_F(5) + 300.0f;
            cXyz temp2;
            MtxPosition(&temp, &temp2);
            temp2 += i_this->field_0x498;
            temp2.y += NREG_F(7) + 300.0f;
        ...
        case 0x2C:
            ...
            if(i_this->mpMorf->isStop()) {
                if(i_this->mAcch.GetGroundH() != -G_CM3D_F_INF) {
                    ...
                    if(dExtKb_GetAttributeCode(i_this->mAcch.m_gnd) == dBgS_Attr_DIRT_e || dExtKb_GetAttributeCode(i_this->mAcch.m_gnd) == dBgS_Attr_GRASS_e || dExtKb_GetAttributeCode(i_this->mAcch.m_gnd) == dBgS_Attr_SAND_e || dExtKb_GetAttributeCode(i_this->mAcch.m_gnd) == dBgS_Attr_WATER_e) {
                        i_this->m5D4[0] = actor->current.pos;
                        i_this->m5EC[0] = actor->current.angle;
        ...
        case 0x2D:
            ...
            cLib_addCalcAngleS2(&actor->shape_angle.x, NREG_F(9) + 10000.0f, 1, NREG_F(10) + 300.0f);
            cLib_addCalc2(&i_this->m4C0, NREG_F(0xB) + -33.0f, 1.0f, NREG_F(0xC) + 1.0f);

            if(std::abs(i_this->m4C0 - (NREG_F(0xB) + -30.0f)) < 1.0f) {
                actor->shape_angle.x = NREG_F(9) + 10000.0f;
                i_this->m4C0 = -33.0f;
                i_this->m554.end();
                i_this->m574.end();
                ...
                if(i_this->m409 != 0) {
                    ...
                    if(pItem) {
                        ...
                        if(actor->home.angle.z == 1) {
                            ...
                            for(int i = 0; i < 10; i++) {
                                fopAcM_create(fpcNm_KS_e, 0x00000007, &actor->current.pos, fopAcM_GetRoomNo(actor), NULL, NULL, 0);
                            }
                        }
                    }
                    else {
                ...
                i_this->m426[0] = NREG_F(0xF) + 40.0f;
                i_this->m420 += 1;
            }
        ...
        case 0x2E:
            if(i_this->m426[0] == 0x23) {
                if(i_this->m407) {
                    pPlayer->changeDemoMode(daPy_demo_c::DEMO_WOLF_MIDNA_RIDE_SHOCK_e, 0, 0, 0);
                }
                else {
                    pPlayer->changeDemoMode(daPy_demo_c::DEMO_UNK_18_e, 0, 0, 0);
                }
            }
            ...
            if(i_this->m426[0] == 0) {
                ...
                i_this->m5D4[1] = actor->current.pos;
                i_this->m5D4[1].y = i_this->mAcch.GetGroundH() + 15.0f + BREG_F(0x11);
                i_this->m5EC[1] = actor->current.angle;

                smoke_set3(i_this);

                actor->speed.y = NREG_F(0xE) + 20.0f;
        ...
        case 0x2F:
            ...
            cLib_addCalc2(&i_this->m4C0, NREG_F(0xB) + 30.0f, 1.0f, NREG_F(0xC) + 10.0f);
            cLib_addCalcAngleS2(&actor->shape_angle.x, 0, 1, 0x1000);

            if(i_this->m426[0] == 0 || (actor->speed.y < 0.0f && i_this->mAcch.GetGroundH() + 10.0f + BREG_F(5) > actor->current.pos.y)) {
                i_this->m574.end();
    ...
    if(i_this->m420 >= 0x2A) {
        ...
        pPlayer->setPlayerPosAndAngle(&pPlayer->current.pos, fopAcM_searchActorAngleY(pPlayer, actor), 0);
    }
}
...
void BG_check(kb_class* i_this) {
    ...
    i_this->mAcchCir.SetWall(i_this->m4D4 * (NREG_F(0x11) + 10.0f) + 25.0f, i_this->m4D4 * 30.0f + 35.0f);
...
static BOOL daKb_Execute(kb_class* i_this) {
    ...
    // §231 P13 acceptance-gate tap — emit on transition of (action=m41E, mode=m420) so
    // state_gate.py can verify the NATIVE pig's state stream against donor-statemap-pig-kb.md
    // (m420 banks 0-6/0xA-D/0x14-19/0x1E-24/0x28-31). Transition-only (low volume).
    if(i_this->m41E != i_this->mDbgLastAction || i_this->m420 != i_this->mDbgLastMode) {
        duskStateTap("pig", i_this->m41E, i_this->m420);
        i_this->mDbgLastAction = i_this->m41E;
        i_this->mDbgLastMode = i_this->m420;
    }
    ...
    if(i_this->mAcch.ChkGroundHit()) {
        ...
        if(i_this->m41E != 1 && i_this->mAcch.GetGroundH() != -G_CM3D_F_INF) {
            ...
            if(dExtKb_GetAttributeCode(i_this->mAcch.m_gnd) == dBgS_Attr_WATER_e) {
                i_this->m404 = 1;
            }
        }
    }
    ...
    if(i_this->m426[5] || (0x21 <= i_this->m420 && i_this->m420 <= 0x24)) {
        i_this->m44C += (s16)(BREG_F(10) + 1000.0f);
        temp2 = 1.0f - (BREG_F(0xB) + 0.5f) * std::abs(cM_ssin(i_this->m44C));
    }
...
static BOOL useHeapInit(fopAc_ac_c* i_actor) {
    ...
    // §229 DN-3: this port never parses BDL at arc-mount — getObjectRes returns RAW
    // bytes. The donor read a pre-fixed J3DModelData; here the model must be parsed
    // at CONSUME time via the mount's acquireModelData (getObjectRes + acquireMountedModel),
    // or McaMorf::create derefs getMaterialNodePointer(0) on raw bytes → crash.
    // §229 color: the pig's COLOR is a MATERIAL swap (pg_*.bmt — TEV/konst colors, e.g.
    // body pink=(246,209,191) vs base=(48,64,64); the bmt has NO textures). Bake the shape's
    // bmt (cache-keyed by model+bmt): applyBodyBmtToModelData copies the material colors and
    // — with the §229 TEX1 guard — keeps the model's own textures, so no index→ptr crash.
    static const char* kKbBmtNames[6] = {
        "pg_pink.bmt",     "pg_buti.bmt",     "pg_kuro.bmt",
        "pg_big_pink.bmt", "pg_big_buti.bmt", "pg_big_kuro.bmt",
    };
    u32 colorIdx = i_this->mShapeType & 3;
    if(colorIdx > 2) {
        colorIdx = 0;
    }
    u32 bmtSel = (i_this->mShapeType >= 8) ? (colorIdx + 3) : colorIdx;
    J3DModelData* pModelData;
    if(i_this->mShapeType >= 8) {
        pModelData = dExtNpcMount_acquireModelDataBmt("Kb", "pg_big.bdl", kKbBmtNames[bmtSel]);
    }
    else {
        pModelData = dExtNpcMount_acquireModelDataBmt("Kb", "pg.bdl", kKbBmtNames[bmtSel]);
    }
    if(pModelData == NULL) {
        return FALSE;  // §229 graceful: a missing pig beats a crash (N31)
    }
    ...
    // §229 DN-3: the BTP eye tex-patterns are RAW resources in this port — calling
    // searchUpdateMaterialID/getUpdateMaterialNum on them derefs raw bytes → crash. The
    // eye-tex application is already a cosmetic no-op (dExtKb_setAnmTexPattern /
    // dExtKb_setTexNoAnimator), so allocate the tex-anm slots off the FIXED model's
    // material count and skip the raw-btp bind entirely.
    u16 materialCount = pModel->getModelData()->getMaterialNum();
    ...
    for(u16 i = 0; i < materialCount; i++) {
        dExtKb_setAnmIndex(&i_this->mpTexNoAnm[i], i);
    }
    ...
    // §229 64-bit fix: donor cast (u32)i_this TRUNCATES the pointer on x86_64 — nodeCallBack
    // then reads back a wild pointer via getUserArea() → C_MTXMultVec AV. Port stores a
    // uintptr_t user-area (like every other ported actor: setUserArea((uintptr_t)this)).
...
static cPhs_Step daKb_Create(fopAc_ac_c* i_actor) {
    ...
    i_this->mDbgLastAction = -1;  // §231 force a tap on the first frame's state
    i_this->mDbgLastMode = -1;

    cPhs_Step status = dComIfG_resLoad(&i_this->mPhs, "Kb");
    if(status == cPhs_COMPLEATE_e) {
        ...
        if(i_this->mbCanBeBigPig != 0xFF) {
            if(strcmp(dComIfGp_getStartStageName(), "sea") == 0 && fopAcM_GetRoomNo(i_actor) == dIsleRoom_OutsetIsland_e) {
                ...
                if(BREG_S(8)) {
                    ...
                    /* §227 BREG_S(8)=0 debug-reg clear — retail HIO no-op (BREG_S is rvalue) */
                }

                if(BREG_S(9) || dComIfGs_isEventBit(dSv_event_flag_c::M_010)) {
                    ...
                    i_this->mShapeType += 8;
                    i_actor->attention_info.distances[fopAc_attn_CARRY_e] = 0x12;
                    fopAcM_OnStatus(i_actor, fopAcStts_UNK_0x10000_e);
                }
                else {
                    if((reg & temp) == 0) {
        ...
        // §231 water fix: donor used 300 (WW's shallow near-shore water). The port's deep
        // Outset sea is a ~5000-unit water column; a 300-unit check can't span it, so a
        // pig thrown into the sea never detects the surface and free-falls to the floor
        // (probe: wet=1 over shallows gnd=-44, wet=0 over deep gnd=-4990). Match the port's
        // proven swimmers (Link + d_a_e_fs both use 10000) so the pig detects deep water.
        i_this->mAcch.SetWaterCheckOffset(10000.0f);
        ...
        i_actor->attention_info.flags = fopAc_AttnFlag_LOCK_e;
