...
// KIT-DONOR: d/actor/d_a_npc_bm1.cpp
...
// ============================================================
// §246 WW Generic Ritos (NPC_BM1..BM5) DIRECT PORT.
// Donor: D:/XXXXXXX/WW DP/src/d/actor/d_a_npc_bm1.cpp - logic is VERBATIM
// ("we ALWAYS do what the donor does"). Only the direct-port crash recipes +
// identifier renames are applied (each marked §246 at its site), mirroring the
// completed d_a_npc_ls1 (Aryll) sister port:
//   cPhs_Step->cPhs_Step; fopAcM_ct->fopAcM_ct; (uintptr_t)this->(uintptr_t)this;
//   dComIfG_Bgsp() reference (-> becomes . ; *deref drops);
//   fopMsgStts_*->fopMsg_MODE_* (§239 talk-path); DEMO_SELECT collapses to retail;
//   DN-3 model load via dExtNpcMount_acquireModelData (parse-at-consume);
//   render #2 (dComIfGd_setList() before entryDL(); morf whole-model calc in setMtx());
//   dComIfGd_setShadow +3 trailing shadow-tex params;
//   port evmng/order arities (getEventIdx +0xFF; orderSpeakEvent +2; orderOtherEventId +4).
//
// §246 BLOCKED SUBSYSTEMS the port lacks (kept donor-faithful via faithful-in-effect
// shims in d_ext_ww_actor_shims.*; these WW Ritos do not spawn in the port so the
// gated branches are runtime-dead): WW save event flags (dSv_event_flag_c UNK_*),
// dSymbol_DIN_e, dBeastIdx_GOLDEN_FEATHER_e, dLetter (dLetter_send), d_snap figurines
// (DSNAP_TYPE_UNK8D..97), WW Rito feather particle FX, fopAcStts_UNK4000_e,
// GP1/ZL1 name sentinels.
// ============================================================
...
#include "d/d_ext_ww_actor_shims.h"   // §246 dSnap_RegistFig / dLetter_send / WW-flag+beast+particle shims
#include "d/d_ext_npc_mount.h"        // §246 DN-3 parse-at-consume model fixup (acquireModelData)
#include "d/d_drawlist.h"             // §246 dDlst_shadowControl_c::getSimpleTex() (port setShadow tail arg)
#include "d/d_com_inf_game.h"         // §246 dComIfGd_setList (render list target)
#include "m_Do/m_Do_ext.h"
#include "m_Do/m_Do_hostIO.h"         // §246 mDoHIO_entry_c / JORMContext (HIO classes)
#include "JSystem/J3DGraphAnimator/J3DJoint.h" // §246 J3DJoint/J3DNode::setCallBack (node callbacks)
...
static BOOL nodeCallBack_Wng(J3DJoint* i_param_1, int i_param_2) {  // §246 J3DNode*->J3DJoint*
    if (i_param_2 == 0) {
...
void daNpc_Bm1_c::nodeWngControl(J3DJoint* i_node, J3DModel* i_model) {
...
static BOOL nodeCallBack_Arm(J3DJoint* i_param_1, int i_param_2) {  // §246 J3DNode*->J3DJoint*
    if (i_param_2 == 0) {
        if (j3dSys.getModel()->getUserArea() != NULL) {
...
void daNpc_Bm1_c::nodeArmControl(J3DJoint* i_node, J3DModel* i_model) {
    s32 uVar1 = ((J3DJoint*)i_node)->getJntNo();
    mDoMtx_stack_c::copy(i_model->getAnmMtx(uVar1));
...
static BOOL nodeCallBack_Bm1(J3DJoint* i_param_1, int i_param_2) {  // §246 J3DNode*->J3DJoint*
    if (i_param_2 == 0) {
        if (j3dSys.getModel()->getUserArea() != NULL) {
...
void daNpc_Bm1_c::nodeBm1Control(J3DJoint* i_node, J3DModel* i_model) {
...
bool daNpc_Bm1_c::chk_appCnd() {
    switch (mSpawnCondition) {
        ...
        case 2:
            if (dComIfGs_isSymbol(dSymbol_DIN_e)) {
                return !dComIfGs_isEventBit(WWEV_UNK_1A80);
            }
            break;
        case 3:
            if (!dComIfGs_isSymbol(dSymbol_DIN_e)) {
            ...
            if (dComIfGs_isEventBit(WWEV_UNK_1A80)) {
...
bool daNpc_Bm1_c::init_PST_2() {
    bool o_retval = false;
    if (!dComIfGs_isEventBit(WWEV_UNK_1F40)) {
...
bool daNpc_Bm1_c::init_PST_3() {
    ...
    if (result) {
        result = dComIfGs_isEventBit(WWEV_UNK_1102) != 0;
        if (result) {
            set_action(&daNpc_Bm1_c::wait_action1, NULL);
        }
    }
...
bool daNpc_Bm1_c::init_PST_4() {
    bool result = dComIfGs_isEventBit(WWEV_UNK_1E80) == 0;
    if (result) {
        m888 = 1;
        gravity = 0.0f;
...
bool daNpc_Bm1_c::init_BMC_1() {
    bool result = false;
    if (!dComIfGs_isEventBit(WWEV_UNK_1808)) {
        result = dComIfGs_isEventBit(WWEV_UNK_1220);
        if (result) {
...
bool daNpc_Bm1_c::init_BMC_2() {
    ...
    if (!dComIfGs_isGetItemReserve(0x0F)) {
        result = dComIfGs_isEventBit(WWEV_UNK_1808);
        if (result) {
...
bool daNpc_Bm1_c::init_BMD_1() {
    if (dComIfGs_isEventBit(WWEV_UNK_1620)) {
        return false;
    }
    return init_BMC_0();
}
...
bool daNpc_Bm1_c::createInit() {
    ...
    static const char* a_staff_tbl[0x10] = {"Bm1", "Bm1", "Bm1", "Bm1", "Bm1", "Bm2", "Bm2", "Bm3", "Bm3", "Bm3", "Bm4", "Bm4", "Bm4", "Bm4", "Bm5", "Bm5"};
    mEventCut.setActorInfo2((char*)a_staff_tbl[mSpecificType], this);
    mAnmNum = 0x16;
...
bool daNpc_Bm1_c::setBtp(bool i_param_1, int i_btp_num) {
    ...
    int iVar1 = mHeadBtpAnm.init(model_data, m_hed_tex_pttrn, 1, 2, 1.0f, 0, -1)  /* §246 port btpAnm::init is 7-arg */;
...
void daNpc_Bm1_c::eventOrder() {
    ...
    if (condition == 1 || condition == 2) {
        ...
        fopAcM_orderOtherEventId(this, mEventIdTable[mEventIdx], 0xFF, 0xFFFF, 4, 1);
    }
}
...
u16 daNpc_Bm1_c::next_msgStatus(u32* i_msg_no) {
    ...
    switch (*i_msg_no) {
        ...
        case 0x199F:
            ...
            break;
        case 0x19A1:
            switch (dExtNpcBm1_msgSelectNum(mpCurrMsg)) {
                case 0:
                    *i_msg_no = 0x19A3;
...
u32 daNpc_Bm1_c::getMsg_PST_1() {
    return dComIfGs_isEventBit(WWEV_UNK_1401) ? 0xC93 : 0xC92;
}

/* 00002B00-00002B40       .text getMsg_PST_3__11daNpc_Bm1_cFv */
u32 daNpc_Bm1_c::getMsg_PST_3() {
    return dComIfGs_isEventBit(WWEV_UNK_2202) ? 0x18A3 : 0x18A1;
}
...
u32 daNpc_Bm1_c::getMsg_SKT_0() {
    if (dComIfGs_isSymbol(dSymbol_DIN_e)) {
        if (!dComIfGs_isEventBit(WWEV_UNK_1A80)) {
            return 0x186B;
        } else if (!dComIfGs_isEventBit(WWEV_UNK_1820)) {
            return 0x1873;
        } else {
            return !dComIfGs_isEventBit(WWEV_UNK_2E04) ? 0x187B : 0x1883;
        }
    }
    return 0;
}
...
u32 daNpc_Bm1_c::getMsg_KKT_0() {
    if (dComIfGs_isSymbol(dSymbol_DIN_e)) {
        if (!dComIfGs_isEventBit(WWEV_UNK_1A80)) {
            ...
            return !dComIfGs_isEventBit(WWEV_UNK_2E04) ? 0x187F : 0x1887;
        }
    }
    return 0;
}
...
u32 daNpc_Bm1_c::getMsg_BMB_2() {
    ...
    if (m87D == 0x47) {
        if (dComIfGs_isEventBit(WWEV_UNK_2180)) {
            return 0x199D;
        }
        if (dComIfGs_isEventBit(WWEV_UNK_2140)) {
            if (dComIfGs_getBeastNum(dBeastIdx_GOLDEN_FEATHER_e) >= 20) {
                return 0x19A1;
            } else {
                return 0x19A0;
        ...
        dComIfGs_onEventBit(WWEV_UNK_2140);
        return 0x199E;
    } else if (m87D != 0xFF) {
        return 0x19A5;
    }
    ...
    if (dComIfGs_isEventBit(WWEV_UNK_2001)) {
        return 0x199A;
    }
    if (dComIfGs_isEventBit(WWEV_UNK_2120)) {
        return 0x1998;
    }
    return 0x1996;
}
...
u32 daNpc_Bm1_c::getMsg_BMC_2() {
    ...
    return dComIfGs_isEventBit(WWEV_UNK_1908) ? 0x1EF2 : 0x1EF0;
}
...
u32 daNpc_Bm1_c::getMsg_BMD_1() {
    ...
    u32 var_r29 = 0;
    if (reg & mask) {
        if (!dComIfGs_isEventBit(WWEV_UNK_1820)) {
            var_r29 = 0x18A5;
        } else {
            switch (mSpawnCondition) {
                case 1:
...
bool daNpc_Bm1_c::chkAttention() {
    dAttention_c& attention = *dComIfGp_getAttention();  // §246 port returns pointer
...
#endif
        delPrtcl_Flyaway();
        mpFlyawayEmitterL = dExtNpcBm1_particleSet(
            BM1_PA_HANEL_FLYAWAY00,
            &mWingLPos,
            &current.angle,
            NULL,
            -1,
            ...
            NULL /*§246 K0 tint deferred*/,
            NULL /*§246 K0 tint deferred*/
        );

        mpFlyawayEmitterR = dExtNpcBm1_particleSet(
            BM1_PA_HANEL_FLYAWAY00,
            &mWingRPos,
            &current.angle,
            NULL,
...
void daNpc_Bm1_c::setPrtcl_Land0() {
    if (mAnmNum == 0xD && mpMorf->checkFrame(1.0f)) {
...
#endif
        delPrtcl_Land0();
        mpLandEmitterL = dExtNpcBm1_particleSet(
            BM1_PA_HANEL_LAND00,
            &mArmLPos,
            &current.angle,
            NULL,
        ...
        mpLandEmitterR = dExtNpcBm1_particleSet(
            BM1_PA_HANEL_LAND00,
            &mArmRPos,
            &current.angle,
            NULL,
...
void daNpc_Bm1_c::setPrtcl_Hane0() {
    delPrtcl_Hane0();
    mpFeatherEmitterL = dExtNpcBm1_particleSet(BM1_PA_TORIZOKU_HANE00, &current.pos, NULL, NULL, 0xFF, NULL, fopAcM_GetRoomNo(this));
    mpFeatherEmitterR = dExtNpcBm1_particleSet(BM1_PA_TORIZOKU_HANE00, &current.pos, NULL, NULL, 0xFF, NULL, fopAcM_GetRoomNo(this));
}
...
void daNpc_Bm1_c::setPrtcl_Hane1() {
    delPrtcl_Hane1();
    mpFeather1EmitterL = dExtNpcBm1_particleSet(BM1_PA_TORIZOKU_HANE01, &current.pos, NULL, NULL, 0xFF, NULL, fopAcM_GetRoomNo(this));
#if VERSION > VERSION_DEMO
    if (mpFeather1EmitterL) {
        mpFeather1EmitterL->becomeImmortalEmitter();
    }
#endif
    mpFeather1EmitterR = dExtNpcBm1_particleSet(BM1_PA_TORIZOKU_HANE01, &current.pos, NULL, NULL, 0xFF, NULL, fopAcM_GetRoomNo(this));
#if VERSION > VERSION_DEMO
...
BOOL daNpc_Bm1_c::isEventEntry() {
    return dComIfGp_evmng_getMyStaffId(mEventCut.getActorName(), this, 0);  // §270 NULL→this (Foundry): same latent staff-claim bug as ba1; untriggered only because the Ritos join no events yet.
}
    ...
    if (dComIfGp_evmng_endCheck(mEventIdTable[mEventIdx])) {
        ...
        switch (mEventIdx) {
            case 0:
                ...
                break;
            case 1:
                dComIfGs_onEventBit(WWEV_UNK_1F40);
                fopAcM_delete(this);
                break;
            case 2:
                dLetter_send(0xAE03);
                dComIfGs_onEventBit(WWEV_UNK_2180);
                break;
            case 3:
                dComIfGs_onEventBit(WWEV_UNK_1E80);
                fopAcM_delete(this);
                break;
        }
        endEvent();
...
BOOL daNpc_Bm1_c::talk_1() {
    ...
    if (mpCurrMsg != NULL) {
        mOldMsgStat = dExtNpcBm1_msgStatus(mpCurrMsg);
    } else {
        mOldMsgStat = 0;
    }
    talk(1);
    if (mpCurrMsg != NULL) {
        switch (dExtNpcBm1_msgStatus(mpCurrMsg)) {
            ...
            case fopMsg_MODE_MSG_DESTROYED_e:
                switch (mCurrMsgNo) {
                    case 0xC92:
                        dComIfGs_onEventBit(WWEV_UNK_1401);
                        break;
                    case 0x18A2:
                        dComIfGs_onEventBit(WWEV_UNK_2202);
                        break;
                    case 0x1997:
                        dComIfGs_onEventBit(WWEV_UNK_2120);
                        break;
                    case 0x19A2:
                        dComIfGs_onEventBit(WWEV_UNK_2001);
                        break;
                    case 0x19A4:
                        m8FD = 5;
                        break;
                    case 0x1EF6:
                        ...
                        break;
                    case 0x1EF8:
                        dComIfGs_onEventBit(WWEV_UNK_1908);
                        break;
                    case 0x1EF7:
                        m87C = 0;
                        break;
...
BOOL daNpc_Bm1_c::wait_2() {
    ...
    if (dComIfGs_isEventBit(WWEV_ENDLESS_NIGHT)) {
        m8FD = 6;
    }
    return TRUE;
}
...
BOOL daNpc_Bm1_c::wait_action2(void*) {
    switch (m904) {
        case 0:
            setStt(0xC);
            if (mSpecificType == SPECIFIC_TYPE_Kogoli_e && dComIfGs_isEventBit(WWEV_UNK_1820) && !dKy_daynight_check()) {
...
bool daNpc_Bm1_c::demo() {
    if (demoActorID == 0) {
        ...
        if (demopattern) {
            ...
            if (mHeadBtpAnm.init(mpHeadMorf->getModel()->getModelData(), m_hed_tex_pttrn, 1, 2, 1.0f, 0, -1)  /* §246 port btpAnm::init is 7-arg */) {
                mBtpNum = 1;
                mBlinkFrame = 0;
            }
        }
        dDemo_setDemoData(this, 0x6A, mpMorf, mArcName, 0, NULL, 0 /*§246 GndMtrlSndId func absent*/, dComIfGp_getReverb(fopAcM_GetRoomNo(this)));  // §246 port arity
    }
    return mbInDemo;
}
...
BOOL daNpc_Bm1_c::_draw() {
    ...
    switch (mType) {
        ...
        case TYPE_Bisht_e:
        case TYPE_Hoskit_e:
            dExtNpcBm1_entryDL_mat(mpMorf, (J3DMaterialTable*)dComIfG_getObjectRes(mArcName, "bm02.bmt"));  // §246 recolor bmt deferred
            break;
        case TYPE_Quill_e:
        case TYPE_Ilari_e:
        case TYPE_Pashli_e:
    ...
    if (mbHasArms) {
        ...
        switch (mType) {
            ...
            case TYPE_Hoskit_e:
                ...
                dExtNpcBm1_entryDL_mat(mpArmMorf, mattable);  // §246 recolor bmt deferred
                break;
            case TYPE_Quill_e:
            case TYPE_Ilari_e:
            case TYPE_Pashli_e:
...
BOOL daNpc_Bm1_c::_delete() {
...
#endif
        if (mpHeadMorf != NULL) {
            dExtNpcBm1_stopZelAnime(mpHeadMorf);
        }
        if (mpWingMorf != NULL) {
            dExtNpcBm1_stopZelAnime(mpWingMorf);
        }
        if (mpArmMorf != NULL) {
            dExtNpcBm1_stopZelAnime(mpArmMorf);
        }
    }
    ...
    delPrtcl_Hane1();
#if VERSION == VERSION_DEMO
    /* §246 l_HIO.removeHIO() is DEBUG-only in the port; dropped */
#endif
    return TRUE;
}
...
#if VERSION == VERSION_DEMO
        /* §246 l_HIO.entryHIO(...) is DEBUG-only in the port (mDoHIO_entry_c #if DEBUG); dropped */
        fopAcM_ct(this, daNpc_Bm1_c);
#endif
...
J3DModelData* daNpc_Bm1_c::create_Anm() {
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "bm.bdl");
...
J3DModelData* daNpc_Bm1_c::create_hed_Anm() {
    ...
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, a_headBDLName_TBL[mSpecificType]);
...
J3DModelData* daNpc_Bm1_c::create_wng_Anm() {
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "bmwing.bdl");
...
J3DModelData* daNpc_Bm1_c::create_arm_Anm() {
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "bmarm.bdl");
...
bool daNpc_Bm1_c::create_itm_Mdl() {
    ...
    if (mType == TYPE_Pashli_e || mType == TYPE_Namali_e) {
        J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "bm_binder.bdl");
    ...
    if (mType == TYPE_Quill_e || mType == TYPE_Ilari_e || mType == TYPE_Pashli_e) {
        J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "bm_bag.bdl");
    ...
    if (mType == TYPE_Akoot_e || mType == TYPE_Skett_e) {
        J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "bm_knife.bdl");
    ...
    if (mType == TYPE_Basht_e || mType == TYPE_Bisht_e || mType == TYPE_Hoskit_e) {
        J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "bm_stick.bdl");
...
static cPhs_Step daNpc_Bm1_Create(fopAc_ac_c* i_this) {
