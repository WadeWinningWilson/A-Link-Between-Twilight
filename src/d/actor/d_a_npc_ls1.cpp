 ...
 *
 * ============================================================
 * §244 DIRECT-PORT RECIPES APPLIED (each marked §244 at its site):
 *   cPhs_State->cPhs_Step; ARRAY_SSIZE->ARRAY_SIZE; (u32)this->(uintptr_t)this;
 *   dComIfG_Bgsp() reference (-> and *deref dropped); fopAcM_ct_Retail->fopAcM_ct;
 *   DEMO_SELECT collapses to retail; talk-path fopMsgStts_*->fopMsg_MODE_* (§239);
 *   DN-3 (three models via dExtNpcMount_acquireModelData + NULL-guard->FALSE);
 *   render #2 (dComIfGd_setList() before mpMorf->entryDL(); the morf whole-model
 *   calc runs in setMtx()); dComIfGd_setShadow +3 trailing shadow-tex params;
 *   port API arities (evmng_getEventIdx +0xFF; orderSpeakEvent +2; orderOtherEventId +4).
 *
 * §244 BLOCKED DEPENDENCIES — this actor is NOT self-contained; it is entangled
 * with two unported subsystems + a sister actor. These refs are kept DONOR-FAITHFUL
 * (unresolved externs). DO NOT wire this TU into files.cmake until they land:
 *   1. daNpc_Bm1_c (d_a_npc_bm1 — Tetra/pirate lookout, telescope partner):
 *      type + setTelescopeDemo()/setFocus()/getOdoroki() + fpcNm_NPC_BM1_e.
 *   2. Telescope/"scope" MESSAGE subsystem: fopMsgM_getScopeMode/releaseScopeMode/
 *      forceSendOn/scopeMessageSet + dComIfGp_getScopeMesgStatus/setScopeMesgStatus/
 *      setScopeType + fopMsgStts_ scope states (MSG_UNK0/CLOSE_WAIT/UNKD/BOX_CLOSED/
 *      BOX_CLOSING).
 *   3. Smaller gaps: dComIfGs_getpCollect()->checkCollect; daPy_getPlayerLinkActorClass
...
#include "d/d_ext_ww_actor_shims.h"  // §244 dSnap_RegistFig etc.
...
// §244 direct-port support headers (mirror pig/seagull):
#include "d/d_ext_npc_mount.h"         // §244 DN-3 parse-at-consume model fixup (acquireModelData)
...
void daNpc_Ls1_matAnm_c::calc(J3DMaterial* i_material) const {
    ...
    for (u32 i = 0; i < 8; i++) {
        // §244 port: getTexMtxAnm(i) returns a reference (never null); the WW
        //       "slot active?" test maps to getTexMtx(i) != NULL (the write
        //       target). getTextureSRT() -> getTexMtxInfo().mSRT (port rename).
...
static BOOL nodeCB_Head(J3DJoint* i_node, int i_calcTiming) {
...
void daNpc_Ls1_c::_nodeCB_Head(J3DJoint* i_bode, J3DModel* i_model) {
...
static BOOL nodeCB_BackBone(J3DJoint* i_node, int i_calcTiming) {
    if (i_calcTiming == J3DNodeCBCalcTiming_In && j3dSys.getModel()->getUserArea() != 0) {
        daNpc_Ls1_c* actor_p = (daNpc_Ls1_c*) j3dSys.getModel()->getUserArea();
...
void daNpc_Ls1_c::_nodeCB_BackBone(J3DJoint* i_node, J3DModel* i_model) {
...
static BOOL nodeCB_Hand_L(J3DJoint* i_node, int i_calcTiming) {
    if (i_calcTiming == J3DNodeCBCalcTiming_In && j3dSys.getModel()->getUserArea() != 0) {
        daNpc_Ls1_c* actor_p = (daNpc_Ls1_c *) j3dSys.getModel()->getUserArea();
...
void daNpc_Ls1_c::_nodeCB_Hand_L(J3DJoint* i_node, J3DModel* i_model) {
    J3DJoint* jnt_p = (J3DJoint *) i_node;
    s32 jnt_no = jnt_p->getJntNo();
...
static BOOL nodeCB_Hand_R(J3DJoint* i_node, int i_calcTiming) {
    if (i_calcTiming == J3DNodeCBCalcTiming_In && j3dSys.getModel()->getUserArea() != 0) {
        daNpc_Ls1_c* actor_p = (daNpc_Ls1_c *) j3dSys.getModel()->getUserArea();
...
void daNpc_Ls1_c::_nodeCB_Hand_R(J3DJoint* i_node, J3DModel* i_model) {
    J3DJoint* jnt_p = (J3DJoint *) i_node;
    s32 jnt_no = jnt_p->getJntNo();
...
static BOOL Ls_hand_nodeCB_Hand_L(J3DJoint* i_node, int i_calcTiming) {
    if (i_calcTiming == J3DNodeCBCalcTiming_In && j3dSys.getModel()->getUserArea() != 0) {
        daNpc_Ls1_c* actor_p = (daNpc_Ls1_c *) j3dSys.getModel()->getUserArea();
...
void daNpc_Ls1_c::_Ls_hand_nodeCB_Hand_L(J3DJoint* i_node, J3DModel* i_model) {
    J3DJoint* jnt_p = (J3DJoint *) i_node;
...
static BOOL Ls_hand_nodeCB_Hand_R(J3DJoint* i_node, int i_calcTiming) {
    if (i_calcTiming == J3DNodeCBCalcTiming_In && j3dSys.getModel()->getUserArea() != 0) {
...
void daNpc_Ls1_c::_Ls_hand_nodeCB_Hand_R(J3DJoint* i_node, J3DModel* i_model) {
    J3DJoint* jnt_p = (J3DJoint *) i_node;
    int jnt_no = jnt_p->getJntNo();
...
void* searchActor_Bm1(void* i_actorP, void* i_unusedP) {
    ...
    if (
        l_check_wrk < ARRAY_SIZE(l_check_inf) && 
...
bool daNpc_Ls1_c::createInit() {
    for (int i = 0; i < ARRAY_SIZE(mEventIDTbl); i++) {
        mEventIDTbl[i] = dComIfGp_evmng_getEventIdx(l_evn_tbl[i], 0xFF);  // §244 port arity (+mapToolID)
    }
    mEventCut.setActorInfo2((char*)"Ls1", this);  // §244 port setActorInfo2 takes char*
    ...
    if (mType) {
        mObjAcch.CrrPos(dComIfG_Bgsp());  // §244 Bgsp() reference (donor *deref drop)
    }
...
u32 daNpc_Ls1_c::getMsg_LS1_0() { 
    if (dComIfGs_isEventBit(WWEV_UNK_0001)) {
        ...
        if (dComIfGs_isEventBit(WWEV_UNK_0280)) {
            return 0xBC9;
        } else {
...
void daNpc_Ls1_c::eventOrder() {
    if (m850 == 1 || m850 == 2) {
        ...
        if (m850 == 1) {
            fopAcM_orderSpeakEvent(this, 0, 0);  // §244 port arity (+priority,+flag)
        }
    } else if (m850 >= 3) {
        mEventIndex = m850 - 3;
        // §244 port arity (+mapToolID,+param3,+param4,+param5) — matches d_a_npc_bou/kolin precedent
        fopAcM_orderOtherEventId(this, mEventIDTbl[mEventIndex], 0xFF, 0xFFFF, 4, 1);
    }
}
...
bool daNpc_Ls1_c::partner_search_sub(fpcLyIt_JudgeFunc i_judgeFunc) {
    ...
    l_check_wrk = 0;
    for (int i = 0; i < ARRAY_SIZE(l_check_inf); i++) {
        l_check_inf[i] = NULL;
    }
...
void daNpc_Ls1_c::setEyeCtrl() {
    for (int i = 0; i < ARRAY_SIZE(mpMatAnms); i++) {
...
bool daNpc_Ls1_c::decideType(int i_type) {
    ...
    switch (i_type) {
        case 0:
            if (dComIfGs_isEventBit(WWEV_UNK_2A80)) {
                mType = 0;
            }    
            break;
        case 1:
...
void daNpc_Ls1_c::privateCut(int i_staffIdx) {
    ...
    mActionIndex = dComIfGp_evmng_getMyActIdx(i_staffIdx, a_cut_tbl, ARRAY_SIZE(a_cut_tbl), TRUE, 0);
    ...
    if (dComIfGp_evmng_endCheck(mEventIDTbl[mEventIndex])) {
        ...
        case 0:
            if (m835) {
                ...
                daPy_getPlayerActorClass()->onNoResetFlg0(daPyFlg0_SCOPE_CANCEL);
                mDoAud_bgmStop(4);
...
bool daNpc_Ls1_c::chkTelescope(cXyz param_1, float param_2, float param_3) {
    ...
    f32 fov_y = dComIfGd_getView()->fovy / 40.0f;
    if (fov_y > 1.0f) {
        fov_y = 1.0f;
    }
...
bool daNpc_Ls1_c::telescope_proc() {
    ...
    case fopMsg_MODE_BOX_CLOSED_e:
        dComIfGp_setScopeMesgStatus(fopMsg_MODE_BOX_CLOSING_e);
        switch (m833) {
        case 3:
            if (m810 != 0) {
                dComIfGp_getCamera(0)->mCamera.clrFlag(0x800000);
                // §244 no port dCamera_c SubjectLock (telescope-dormant): dComIfGp_getCamera(0)->mCamera.SubjectLockOff();
                dComIfGs_onEventBit(WWEV_UNK_0310);
                mTimer5 = 0;
                m833 = 4;
                fopMsgM_getScopeMode();
                return true;
    ...
    case fopMsg_MODE_SCOPE_WAIT_e: 
        {
            ...
            case 2:
                ...
                if (dComIfGp_getCameraZoomScale(0) >= l_HIO.mPrm.m28) {
                    if (scope_mesg_status == fopMsg_MODE_CLOSE_WAIT_e) {
                        fopMsgM_forceSendOn();
                    }
            ...
            if ((m833 == 1 && temp_msg_no != m810) || cLib_calcTimer(&mTimer5) == 0) {
                if (scope_mesg_status == fopMsg_MODE_CLOSE_WAIT_e) {
                    ...
                    if (m833 == 1 && m810 == 0xBCB) {
                        // §244 no port dCamera_c SubjectLock (telescope-dormant): dComIfGp_getCamera(0)->mCamera.SubjectLockOn(bm1_npc_p);
                        m810 = 0xBCA;
                        mTimer5 = 150;
                        m833 = 2;
                    }
...
#else
        dDemo_setDemoData(this, 106, mpMorf, mArcName, 0, NULL, 0, 0);  // §244 port arity (8-arg: +GndMtrlSndId, +reverb)
#endif
    }
...
void daNpc_Ls1_c::shadowDraw() {
    ...
    mShadowID = dComIfGd_setShadow(
        ...
        // §244 port setShadow adds 3 trailing params (shadow-tex); standard actor
        // values, matches d_a_kb / d_a_kamome / alink.
...
BOOL daNpc_Ls1_c::_draw() {
    ...
    // McaMorf ANIM/morph blend, NOT the model render calc — so the body must
    // modelCalc() here (as the pig d_a_kb does), else nothing renders.
    // ORDER MATTERS: modelCalc() re-runs the material calc, which resets the
    // texture pattern — so the FACE btp/btk must be entered AFTER modelCalc and
    // before entryDL, or the eye/mouth panes fall back to their blank default.
    mpMorf->modelCalc();
...
BOOL daNpc_Ls1_c::_delete() {
    ...
    if (mbResLoadIsComplete) {
        /* §244 removeHIO() is DEBUG-only in the port; dropped */
#endif
...
cPhs_Step daNpc_Ls1_c::_create() {
    ...
    fopAcM_ct(this, daNpc_Ls1_c);  // §244 WW fopAcM_ct_Retail -> port fopAcM_ct
...
#if VERSION == VERSION_DEMO
    // §244 DEBUG-only in port (dropped): l_HIO.entryHIO("アリル"); // Aryll
...
BOOL daNpc_Ls1_c::bodyCreateHeap() {
    // §244 DN-3: parse-at-consume. The BODY model feeds a McaMorf ctor that derefs
    // getMaterialNodePointer(0); a raw getObjectIDRes buffer -> AV. Route through
    // acquireModelData (fixed J3DModelData). NULL-guard -> FALSE.
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "ls.bdl");
    if (a_mdl_dat == NULL) {
        return FALSE;
    }
...
BOOL daNpc_Ls1_c::handCreateHeap() {
    mpLsHandModel = NULL;
    // §244 DN-3: parse-at-consume for the ls-hand model. NULL-guard -> FALSE.
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "lshand.bdl");
    if (a_mdl_dat == NULL) {
        return FALSE;
    }
...
BOOL daNpc_Ls1_c::itemCreateHeap() {
    ...
    // §244 DN-3: parse-at-consume for the telescope model. WW loads it from the
    // "Link" arc (dComIfG_getObjectRes("Link", dRes_INDEX_LINK_BDL_TELESCOPE_e)),
    // but the port has no plain "Link" arc (only LinkRM/LinkUG) — the mod bundles
    // telescope.bdl into the Ls arc (npc_ls.ini attach_model), the same arc the
    // body + hand load from. Source it from mArcName. NULL-guard -> FALSE.
    J3DModelData* a_mdl_dat = dExtNpcMount_acquireModelData(mArcName, "telescope.bdl");
    if (a_mdl_dat == NULL) {
        return FALSE;
    }
...
static cPhs_Step daNpc_Ls1_Create(fopAc_ac_c* i_this) {
