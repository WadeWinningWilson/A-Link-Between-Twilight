...
void dExtWwCam_installData(void) {
    ...
    {
        DuskLog.error("[WwCamData] style blob framing BAD (magic/count/size) — regenerate "
                      "ww_cam_data.inc");
        ok = false;
    }
    ...
    for (int i = 0; ok && i < (int)(sizeof(kSpots) / sizeof(kSpots[0])); i++) {
        ...
        if (std::memcmp(rec, kSpots[i].fourCC, 4) != 0 || alg != 20 || flags != kSpots[i].flags) {
            DuskLog.error("[WwCamData] spot-check {} FAILED (alg={} flags={:#x}) — regenerate "
                          "ww_cam_data.inc",
    ...
    if (ok) {
        DuskLog.info("[WwCamData] resident: {} donor styles ({} bytes, camstyle.dat-shaped), "
                     "{} donor-axis types, {} BG type names. Self-test OK. Selects NOTHING "
                     "yet — the donor mode selector is the next port (FULL ruling).",
