...
// KIT-DONOR: JSystem/JAudio/JASSeqParser.cpp
// KIT-DONOR-REF: zeldaret/tww@1d57f0468986987ec26a3d1800bdc1aaad3794db
// KIT-DONOR-STATUS: Matching
// ============================================================
// §367 JAudio1 sequence-command parser — donor-verbatim port of WW
// retail JSystem/JAudio/JASSeqParser.cpp (D:/XXXXXXX/WW DP, fully
// matched TU; dtk address markers kept). Phase A2 of the native JA1
// sequencer campaign (bus §362/§363). Donor JASystem::* flattens into
// JAudio1::* (§363 convention, §328 same-name lesson) — donor symbol
// names kept. Nothing in the game calls this TU yet; the ExtSeq bridge
// keeps running until A4 swaps entry points.
// Donor include map (§367):
//   JASSeqParser.h            -> d/ext_seq/ja1_jasseqparser.h
//   JASTrack.h                -> d/ext_seq/ja1_jastrack.h
//   JASPlayer.h / JASChannel.h / JSupport.h / JUTAssert.h
//                             -> d/ext_seq/ja1_boundary.h (§367 layer)
// Per-line adaptations are each marked // ===== §367 at the site; the
// only ones in this TU are the two x64 pointer/int cast bridges
// (cmdJmp, cmdPrintf).
// ============================================================

#include "d/ext_seq/ja1_jasseqparser.h"
#include "dusk/logging.h"  // §374 probes
#include "d/ext_seq/ja1_jastrack.h"

#include "dolphin/os.h"  // OSReport (donor: dolphin/os/OS.h)

JAudio1::TSeqParser::CmdFunc JAudio1::TSeqParser::sCmdPList[64] = {
    NULL,
    &JAudio1::TSeqParser::cmdOpenTrack,
    &JAudio1::TSeqParser::cmdOpenTrackBros,
    NULL,
    &JAudio1::TSeqParser::cmdCall,
    NULL,
    &JAudio1::TSeqParser::cmdRet,
    NULL,
    &JAudio1::TSeqParser::cmdJmp,
    &JAudio1::TSeqParser::cmdLoopS,
    &JAudio1::TSeqParser::cmdLoopE,
    &JAudio1::TSeqParser::cmdReadPort,
    &JAudio1::TSeqParser::cmdWritePort,
    &JAudio1::TSeqParser::cmdCheckPortImport,
    &JAudio1::TSeqParser::cmdCheckPortExport,
    &JAudio1::TSeqParser::cmdWait,
    NULL,
    &JAudio1::TSeqParser::cmdParentWritePort,
    &JAudio1::TSeqParser::cmdChildWritePort,
    NULL,
    &JAudio1::TSeqParser::cmdSetLastNote,
    &JAudio1::TSeqParser::cmdTimeRelate,
    &JAudio1::TSeqParser::cmdSimpleOsc,
    &JAudio1::TSeqParser::cmdSimpleEnv,
    &JAudio1::TSeqParser::cmdSimpleADSR,
    &JAudio1::TSeqParser::cmdTranspose,
    &JAudio1::TSeqParser::cmdCloseTrack,
    &JAudio1::TSeqParser::cmdOutSwitch,
    &JAudio1::TSeqParser::cmdUpdateSync,
    &JAudio1::TSeqParser::cmdBusConnect,
    &JAudio1::TSeqParser::cmdPauseStatus,
    &JAudio1::TSeqParser::cmdSetInterrupt,
    &JAudio1::TSeqParser::cmdDisInterrupt,
    &JAudio1::TSeqParser::cmdClrI,
    &JAudio1::TSeqParser::cmdSetI,
    &JAudio1::TSeqParser::cmdRetI,
    &JAudio1::TSeqParser::cmdIntTimer,
    &JAudio1::TSeqParser::cmdVibDepth,
    &JAudio1::TSeqParser::cmdVibDepthMidi,
    &JAudio1::TSeqParser::cmdSyncCPU,
    &JAudio1::TSeqParser::cmdFlushAll,
    &JAudio1::TSeqParser::cmdFlushRelease,
    &JAudio1::TSeqParser::cmdWait,
    &JAudio1::TSeqParser::cmdPanPowSet,
    &JAudio1::TSeqParser::cmdIIRSet,
    &JAudio1::TSeqParser::cmdFIRSet,
    &JAudio1::TSeqParser::cmdEXTSet,
    &JAudio1::TSeqParser::cmdPanSwSet,
    &JAudio1::TSeqParser::cmdOscRoute,
    &JAudio1::TSeqParser::cmdIIRCutOff,
    &JAudio1::TSeqParser::cmdOscFull,
    &JAudio1::TSeqParser::cmdVolumeMode,
    &JAudio1::TSeqParser::cmdVibPitch,
    NULL,
    NULL,
    NULL,
    NULL,
    NULL,
    &JAudio1::TSeqParser::cmdCheckWave,
    &JAudio1::TSeqParser::cmdPrintf,
    &JAudio1::TSeqParser::cmdNop,
    &JAudio1::TSeqParser::cmdTempo,
    &JAudio1::TSeqParser::cmdTimeBase,
    &JAudio1::TSeqParser::cmdFinish,
};

const JAudio1::Arg_s JAudio1::Arglist[0x40] = {
    {0x0000, 0x0000},
    {0x0002, 0x0008},
    {0x0002, 0x0008},
    {0x0001, 0x0002},
...
int JAudio1::TSeqParser::cmdOpenTrack(TTrack* track, u32* args) {
    u32 param1 = args[0];
    u32 param2 = args[1];
    u8 b1 = param1 & 0xF;
    u8 b2 = (param1 >> 6) & 3;
...
int JAudio1::TSeqParser::cmdOpenTrackBros(TTrack* track, u32* args) {
    if (!track->getParent()) {
...
int JAudio1::TSeqParser::cmdCall(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdRet(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdJmp(TTrack* track, u32* args) {
    ...
    if (conditionCheck(track, flag)) {
        if (usetrackptr) {
            // ===== §367: donor casts the 32-bit register value straight to a
            // pointer (`(void*)trackptr`) — GC pointers were 32-bit. The u64
            // hop silences MSVC C4312 on x64; the seq-register track-pointer
            // register cannot carry a real PC pointer until A3/A4 define the
            // register width story.
            track->getSeq()->start((void*)(u64)trackptr, 0);
        } else {
...
int JAudio1::TSeqParser::cmdLoopS(TTrack* track, u32* args) {
    u32 timer = args[0];
...
int JAudio1::TSeqParser::cmdLoopE(TTrack* track, u32* args) {
    track->getSeq()->loopEnd();
    return 0;
}

/* 8027EB54-8027EBAC       .text cmdReadPort__Q28JASystem10TSeqParserFPQ28JASystem6TTrackPUl */
int JAudio1::TSeqParser::cmdReadPort(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdWritePort(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdParentWritePort(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdChildWritePort(TTrack* track, u32* args) {
    u8 param1 = args[0];
...
int JAudio1::TSeqParser::cmdCheckPortImport(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdCheckPortExport(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdWait(TTrack* track, u32* args) {
    u32 param1 = args[0];
...
int JAudio1::TSeqParser::cmdSetLastNote(TTrack* track, u32* args) {
    u32 key = args[0];
...
int JAudio1::TSeqParser::cmdSimpleADSR(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdTranspose(TTrack* track, u32* args) {
    track->setTranspose(args[0]);
    return 0;
}

/* 8027EF3C-8027EFD8       .text cmdCloseTrack__Q28JASystem10TSeqParserFPQ28JASystem6TTrackPUl */
int JAudio1::TSeqParser::cmdCloseTrack(TTrack* track, u32* args) {
    u8 track_no = args[0];
...
int JAudio1::TSeqParser::cmdOutSwitch(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdDisInterrupt(TTrack* track, u32* args) {
    track->mIntrMgr.resetInter(args[0]);
    return 0;
}

/* 8027F10C-8027F124       .text cmdClrI__Q28JASystem10TSeqParserFPQ28JASystem6TTrackPUl */
int JAudio1::TSeqParser::cmdClrI(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdSetI(TTrack* track, u32* args) {
    track->mIntrMgr.disable();
    return 0;
}

/* 8027F134-8027F178       .text cmdRetI__Q28JASystem10TSeqParserFPQ28JASystem6TTrackPUl */
int JAudio1::TSeqParser::cmdRetI(TTrack* track, u32* args) {
    track->mIntrMgr.enable();
...
int JAudio1::TSeqParser::cmdFIRSet(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdEXTSet(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdPanSwSet(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdOscRoute(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdVibDepth(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdVibDepthMidi(TTrack* track, u32* args) {
    f32 depth = args[0];
    depth /= 393204.0f;
    track->mVibrate.setDepth(depth);
    return 0;
...
int JAudio1::TSeqParser::cmdVibPitch(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdIIRSet(TTrack* track, u32* args) {
    for (u8 i = 0; i < 4; i++) {
...
int JAudio1::TSeqParser::cmdIIRCutOff(TTrack* track, u32* args) {
    u8 iirTableIdx = args[0];
    for (u8 i = 0; i < 4; i++) {
        s16* table = &JAudio1::Player::CUTOFF_TO_IIR_TABLE[iirTableIdx * 4];
        u8 iirIndex = TTrack::TIMED_IIR_Unk0 + i;
        TTrack::MoveParam_* iir = &track->mTimedParam.mMoveParams[iirIndex];
...
int JAudio1::TSeqParser::cmdOscFull(TTrack* track, u32* args) {
...
int JAudio1::TSeqParser::cmdCheckWave(TTrack* track, u32* args) {
    track->mRegisterParam.setFlag(0);
    return 0;
}

/* 8027F6A8-8027F8F4       .text cmdPrintf__Q28JASystem10TSeqParserFPQ28JASystem6TTrackPUl */
int JAudio1::TSeqParser::cmdPrintf(TTrack* track, u32* args) {
    char buf[128];
    u8 byteArray[4];
    int registers[4];
    u32 count = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (byteArray[i] == 2) {
            // ===== §367: donor truncates the seq-data pointer into its int
            // register slot (`(int)track->getSeq()->getAddr(...)`) for the
            // %s case — GC pointers were 32-bit. The u64 hop silences MSVC
            // C4311 on x64; a truncated %s argument cannot round-trip on PC
            // (donor debug-print path only; nothing calls this TU yet).
            registers[i] = (int)(u64)track->getSeq()->getAddr(registers[i]);
...
int JAudio1::TSeqParser::Cmd_Process(TTrack* track, u8 r5, u16 r6) {
    JAudio1::Arg_s sp08 = Arglist[r5-0xC0];
    u16 r29 = sp08.m02;
    r29 |= r6;
    u32 sp18[8];
    ...
    JAudio1::TSeqParser::CmdFunc cmdFunc = sCmdPList[r5-0xC0];
...
int JAudio1::TSeqParser::RegCmd_Process(TTrack* track, int r30, int r31) {
...
int JAudio1::TSeqParser::cmdSetParam(TTrack* track, u8 param_2) {
    u8 flag = track->getSeq()->readByte();
...
int JAudio1::TSeqParser::cmdWait(TTrack* track, u8 flag) {
...
int JAudio1::TSeqParser::cmdNoteOff(TTrack* track, u8 flag) {
    if (flag == 0xF9) {
...
int JAudio1::TSeqParser::cmdNoteOn(TTrack* track, u8 note) {
#if TARGET_PC
    // §374a parser-stage probe (strip at A5): does the READER produce notes?
    { static int n = 0; if (n < 6) { ++n; DuskLog.info("[JA1] §374a cmdNoteOn #{} note={}", n, (int)note); } }
#endif
    ...
    if (track->mNoteMgr.getConnectCase() & 0x2) {
        ...
        JAudio1::TChannel* channel = track->mNoteMgr.getChannel(0);
        if (channel) {
...
bool JAudio1::TSeqParser::conditionCheck(TTrack* track, u8 param_2) {
...
int JAudio1::TSeqParser::parseSeq(TTrack* track) {
    while (true) {
        u8 flag = track->getSeq()->readByte();
#if TARGET_PC
        // §374b opcode-stream trace (strip at A5): the first 24 bytes the NATIVE
        // reader dispatches — diffable against the bridge parser's golden stream.
        { static int n = 0; if (n < 24) { ++n; DuskLog.info("[JA1] §374b op[{}]={:#04x}", n, (int)flag); } }
#endif
        u32 r29 = 0;
        if (!(flag & 0x80)) {
