...
//
// STEP 19: THIS TU MOVES TO THE PLUGIN. Established by comparing against
// dusklight main rather than by reading the code, which misled me twice:
//   - dusklight main has NO d_npc.cpp at all, and none in its files.cmake
//   - its c_lib.cpp IS decompiled and HAS cLib_addCalcAngleS/S2 -- but no
//     cLib_addCalcAngleL, so the s32 variant this file calls is a WW function,
//     not a TP one our decomp merely hasn't reached
//   - every user of dNpc_c is a WW actor already in the exclusion roster
//
// The code READS as TP framework because it is written in TP's idiom, and I
// argued twice from that appearance that it was receiver-native. Appearance is
// not provenance; the comparison against dusklight main is.
//
// Generated by dtk
...
#include "d/d_ext_ww_actor_shims.h" // §239 WW NPC framework donor-gap shims
...
void dNpc_setShadowModel(J3DModel* param_1, J3DModelData* param_2, J3DModel* param_3) {
    for(int i = 0; i < param_2->getWEvlpMtxNum(); i++) {
        dExtNpc_setWeightAnmMtx(param_1, i, param_3->getWeightAnmMtx(i)); // §239 adapter (port J3DModel lacks setter)
    }
...
bool dNpc_chkArasoi() {
    // §239 WW event flags mapped to the port's label at the SAME value (pig doctrine
    // UNK_3402->F_0428): UNK_1220==0x1220->F_0049, UNK_1808==0x1808->F_0202.
    return dComIfGs_isEventBit(dSv_event_flag_c::F_0049) && !dComIfGs_isEventBit(dSv_event_flag_c::F_0202);
}
...
u16 fopNpc_npc_c::talk(int param_1) {
    ...
    if(mCurrMsgBsPcId == fpcM_ERROR_PROCESS_ID_e) {
        ...
        // §239 port fopMsgM_messageSet takes (msgIdx, talkActor, param_2); param_2 =
        // 1000 matches every port call site (d_msg_flow etc.).
        mCurrMsgBsPcId = fopMsgM_messageSet(mCurrMsgNo, this, 1000);
        mpCurrMsg = NULL;
    }
    else {
        if(mpCurrMsg) {
            // §239 WW msg_class::mStatus -> port msg_class::mode; fopMsgStts_* -> fopMsg_MODE_*.
            status = mpCurrMsg->mode;
            if(status == fopMsg_MODE_MSG_DISPLAYED_e) {
                mpCurrMsg->mode = next_msgStatus(&mCurrMsgNo);
                if(mpCurrMsg->mode == fopMsg_MODE_MSG_CONTINUE_e) {
                    fopMsgM_messageSet(mCurrMsgNo, 1000); // §239 port 2-arg overload (msgIdx, param_1)
                }
            }
            else if (status == fopMsg_MODE_BOX_CLOSED_e) {
                mpCurrMsg->mode = fopMsg_MODE_MSG_DESTROYED_e;
