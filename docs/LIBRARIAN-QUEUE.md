...
era: era-independent
<!-- era rationale: a routing queue, not a method | Librarian, 2026-08-16, user ruling "assign it by era" -->
...
frozen fork **A** [WW-Restoration-Cookbook-SUPERSEDED.md](WW-Restoration-Cookbook-SUPERSEDED.md) ·
frozen fork **B** [WW Linked/WW-Restoration-Cookbook-SUPERSEDED.md](WW%20Linked/WW-Restoration-Cookbook-SUPERSEDED.md).
...
> **RULING CONFIRMED to Librarian, live in-chat 2026-08-14 (tale §939):** user confirmed "THE PLUGIN IS THE PRODUCT, mod-side delivery only" — AND clarified a nuance I inscribed prominently so no lane over-applies §939: **receiver code stays as-is in CONTENT (it makes the mod viable); only DELIVERY moves plugin-side — preserved + delivered by the plugin, NOT deprecated.** Verified the claimed ruling with the real user before recording (did not ratify the board's "USER RULING" row on its own claim — same discipline as the §934 retraction lesson). Updated [AGENT_INDEX](AGENT_INDEX.md) clean-room row OPEN→RULED. **Flagged + routing to Bridge/Foundry:** the design crux the clarification surfaces — the proven mod shape (`ww_donor_disc`) delivers DATA with zero receiver headers, but the ruling needs receiver *logic* delivered by the plugin; that gap is the feasibility question (Foundry already measuring Tier-2). Not mine to design.
> **§933 CLEAN-ROOM RUN indexed 2026-08-14 (Integrator, CALLS row 114):** the plugin's **data-delivery half is PROVEN on a provenance-verified VANILLA exe** — `ww_donor_disc.dusk` served 224 donor files zero-change, `[V10a]`=0 ⇒ receiver layer absent ⇒ **§934 confirmed**; receiver/routing half still the gap (Foundry Tier-2, gate 0-B). Verified the header claim against the body before indexing (header "PLUGIN WORKS ON STOCK" is scoped in-row to donor-ACCESS only — no §934 contradiction; the Integrator tagged the scope, exactly the schema discipline). Updated [AGENT_INDEX](AGENT_INDEX.md) clean-room row. **STALE-CONTENT FLAG (route to owner Housing/Integrator, NOT mine to edit):** [`ww-clean-room-delivery.md`](WW%20Linked/ww-clean-room-delivery.md) §(2) still reads "NO — never exercised end to end"; the DATA half is now exercised (§933) — the owner should append the result so the durable doc doesn't contradict the hub. Low-urgency; not filed as a formal row (owner just ran it, clearly aware).
> **§953 hook-path finding NOTED, NOT yet indexed (Integrator, CALLS row 123, 2026-08-14):** the by-name hook path resolves against a **symgen manifest embedded inside `dusklight.exe`** (keyed to that exe's own PDB GUID+age), so a `DEFINE_HOOK_SYMBOL` plugin bakes no addresses / ships no symbol file ⇒ **version-independent BY CONSTRUCTION** — a structurally stronger delivery story than §934 could claim. **HELD from the hub on purpose:** it rests on **H10 (unverified)** — whether `FEATURES game` (mandatory for hooking; `hook.hpp:3`) link-couples the DLL to `dusklight_imports.lib`; if H10 fails the version-independence is LINK-coupled and narrower. Integrator states it as hypothesis, checkable by `dumpbin /imports ww_hook_smoke.dll`. **Also surfaced (owner's, not mine): a release-step gate — a QUIET `SymbolManifest.cmake:64` path can ship a user build with no embedded manifest ⇒ every by-name hook fails silently.** INDEX the RESOLVED story into the clean-room hub row once H10 is checked; do not fold hypothesis into hub voice before then (§934 lesson). ⟶ **RESOLVED + INDEXED same-day (Integrator run-180329, CALLS row 208):** H10 CLOSED — DLL imports only KERNEL32/VCRUNTIME/CRT + itself, no thunk into the exe; hook path proven END-TO-END on the same vanilla binary (resolve→fire→HOOK_CONTINUE→balanced calls). Folded into [AGENT_INDEX](AGENT_INDEX.md) clean-room row **with the Integrator's own scope preserved**: ONE uniquely-named hook only; the §747 registration chain (proc/slot/placement) is the ACTUAL Tier-2 blocker and is untouched; 49-file receiver layer still in-exe. **The hold-until-verified discipline worked exactly as intended** — hypothesis stayed out of hub voice for one turn, went in only once measured.
> **§968 OPTION 1 — board-claimed USER RULING, NOT yet inscribed (CALLS row 234, via Housing, 2026-08-14):** WW actors claim profile indices ABOVE the host proc enum (no receiver edit, no slot budget, works on stock); options 2/3/4 rejected (4 = "recompiling the user's exe = fork delivery, the thing the plugin replaces"); 13-slot ceiling + 46-actor Outset countdown GONE. Vehicle `mods-src/ww_registry/` landed DARK (inert empty table, gate-0-B clean, queued-not-built §839, both symbols pre-flight SAFE fork+vanilla). **⚠ §455 REFINEMENT (relay corrected): `--symbol SAFE` proves NAME RESOLUTION ONLY, not SIGNATURE COMPATIBILITY** — Housing found `fopAcM_GetParamBit` diverges donor (u32 *value*) vs receiver (`void*` *actor ptr*): same name, compiles, wild-reads on the first real actor. A binding now needs TWO verdicts — `--symbol` SAFE on both images **AND** donor-vs-receiver signature comparison at source (Integrator's adopted gate; it is Foundry's doctrine, not mine to codify). So "pre-flight SAFE" above is NOT "signature-cleared." **AWAITING real-user confirmation before I inscribe "slot question CLOSED" into the hub** (board "USER RULING" ≠ consent — §939 discipline). **Self-note: the AGENT_INDEX clean-room cell is bloating** (§933 + §953/H10 + would-be §968 all appended to one cell) → compaction candidate: keep the hub a concise pointer, let the full chain live in `ww-clean-room-delivery.md`. Do once confirmed + out of the flurry.
> **§975 VISIBILITY + PROTOCOL (CALLS rows 253/254, 2026-08-14):** was invisible to every lane-liveness instrument (worked only in CALLS.md/QUEUE/AGENT_INDEX, never posted a bus header). FIXED: allocated **tale §976** → posted first `## §N LIBRARIAN` bus section (content-neutral pointers) → **verified via `call_receipt.py` (`LIBRARIAN@§976` now in "lanes seen")**. Hit an allocator §976 double-issue (History held it too, posted first, unledgered) → took **§976b** (second-occurrence suffix, my own convention against myself), touched no History content, filed the race to Foundry as a NEW ROW. Codified row-254's rule into CALLS Protocol: **APPEND-IS-NOT-A-CALL-SURFACE** (new ask ⇒ new row) — dogfooded it by filing the Foundry flag as a new row. Operating-model change persisted to [[reference_librarian_ops]] (post a bus section per session for visibility; detection via CALLS.md unchanged). **Cite the lane as `tale §976b`.**
> **§428/run-203926 THIRD DELIVERY PROOF indexed + hub COMPACTED (2026-08-14):** above-enum DISPATCH works on stock/vanilla — all 3 §747 legs install, a profile index at 4096 routed to plugin data (§968 Opt-1 mechanism demonstrated). Completes the trilogy: DATA serves (§933) · HOOKS fire (§953/H10) · DISPATCH routes (§428). Indexed with the Integrator's scope preserved: **mechanism proven ≠ WW actor created** — pilot is a LINKAGE PROBE not a port (Housing self-applied DN-10-S); remaining = CARGO (real actor + behaviour + placement); 49-file receiver layer still in-exe. **Indexed the PROOF, NOT the §968 ruling** (ruling stays HELD pending user confirm — a board demonstration of the mechanism ≠ user confirming the decision). **Q2 (hub bloat) largely RESOLVED:** did a light in-place compaction of the AGENT_INDEX clean-room cell (2356→1408 bytes) — three proofs now one-liners, detail preserved in CALLS rows + this queue; the heavier "move full chain into the delivery doc" version is optional, no longer blocking.
> **BORN-CHECKED ROW = 7th delivery-failure shape, codified into CALLS Protocol (Housing row 516 + Integrator row 535, 2026-08-14):** the first SENDER-side shape — a row filed `[x]` at birth is invisible to every monitor by construction (Housing self-reported 39/45; Integrator notes it's not measurable from the file since CALLS.md isn't committed — credible self-report, not measured). **This is the ROOT CAUSE of the user's recurring "are my calls landing?" question (asked 3×).** Two lanes converged (Housing filing-side, Integrator detection-side) on the same rule I codified: **"THE CHECKBOX IS ADDRESSED TO THE READER, NOT A NOTE TO SELF — file new asks `[ ]`; check only to DISCHARGE; pure records labelled RECORD"** (CALLS Protocol; `file_row.py add` enforces `[ ]` by construction). **RECEIVER-SIDE SAFETY SCAN CLEAN:** no `[x] LIBRARIAN` row lacks a `->` discharge, so no born-checked call to me was missed. My own filings were always `[ ]`→`[x]`+`->` (proper).
> **§559/run-214437 FIRST DONOR-FAITHFUL ACTOR registered on stock (Integrator, 2026-08-14):** `tag_so` port REGISTERED + READY on vanilla (process_size 1656, 3 placement routes, hooks MOD_OK) — supersedes the linkage-probe. Indexed into hub with the Integrator's scope preserved: **registered ≠ created; INERT for want of a placement.** Boundary: stock can LOAD our actor but cannot ASK for it (asking needs a stage = receiver layer). **OPEN DECISION surfaced to user (Integrator put it up for ruling — decides the next several passes):** (a) prove create+getArg on the FORK (real DZR, but a binary that already has the receiver layer) vs **(b) scope whether the plugin can INJECT a placement on vanilla — "the actual delivery question, nobody has scoped."** (b) is the one that matters for the user's product (vanilla + plugin + ISO). Not mine to rule; routed to user. ⟶ **RULED route (b) §591 (board-claimed, user via Foundry; overruled Bridge's (a)-first): b1→b2→b3 sequential** — b1 (data→vanilla) DONE, b2 (`dComIfGp_setNextStage` exported/uncalled) first work, **b3 (parse+enter WW stage = the 49-file receiver layer) = THE unknown whose outcome = migration scope.** Reflected in hub, attributed/board-claimed.
> **HANDOFF written (§589 user order, 2026-08-14):** [`docs/HANDOFF-LIBRARIAN.md`](HANDOFF-LIBRARIAN.md) — six sections, §-stamped, pointers-not-copies, tools verified present. Cross-read requested from Foundry (pairing Librarian↔Foundry) via `file_row.py`. **Error self-classification: mostly-METHODOLOGY, one genuine error (row-81) → per the directive's logic, control-not-retiring for this lane; no LOST-TRACK context-degradation observed.** **§589 ROSTER CORRECTION folded in: HOUSINGTEMP = INTEGRATOR (same instance, "five identities not six")** — this ANSWERS the INTEGRATOR/HOUSINGTEMP half of LANES-ROSTER-DRIFT and confirms **my row-81 normalization was WRONG** (HOUSINGTEMP is Integrator, not Housing/Engine). Recorded in the handoff's DO-NOT-TRUST. **HELD from LANES.md pending real-user confirm** (board-claimed); still open: HOUSING vs Housing Security canonical token, + whether to inscribe.
...
| CALLS-LANE-TOKEN | LINT DOCTRINE | Every `CALLS.md` row's **lane field** (text before the first `\|`, after the checkbox) must match the **known-lane set in [`docs/LANES.md`](LANES.md)**. A lane token outside that set silently addresses NOBODY and defeats every lane-anchored filter (`^- \[ \] HOUSING/ENGINE`). **Rule stated by Librarian (doctrine); the check is Bridge/Foundry's to build — I operate, don't build, linters.** First live case 2026-08-14: CALLS row 81 lane read `HOUSINGTEMP` (already `[x]`, so not operationally stuck) → normalized to `HOUSING/ENGINE` (row's own answer signed "TAKEN by Housing/Engine (tale §925)", intended lane unambiguous). Caught by Integrator. My own librarian monitor is a case-insensitive `librarian` substring, immune to this class — the rule protects the OTHER lanes' anchored filters. **CORROBORATED + ESCALATED 2026-08-14 (Housing, CALLS row 285):** Housing independently named this doctrine THE source-level fix for a **5-shape family of silent delivery-failures** (case-sensitive lane matcher, header-count change-detection, broken dedup, ask-on-checked-row, unwritten routing) — "if that lint runs, mixed-case and unknown addressees stop existing at the source and no monitor needs to be clever." Escalated to a concrete build-request: enforce it in **`file_row.py add`** (reject lane ∉ `docs/LANES.md`) — the filer already does ambiguity-refusal + byte-hygiene, so it is the same chokepoint; filed to Foundry via `file_row.py` itself. | DOCTRINE STATED · **BUILT into `file_row.py add` (Foundry, tale §977) — rejects non-canonical, corpus lint-clean** |
| LANES-ROSTER-DRIFT | ROSTER CURRENCY (routed) | **The authority and the enforcement disagree, and I acted on the wrong one at row 81.** `CALLS-LANE-TOKEN` cites `docs/LANES.md` as the known-lane authority; LANES.md lists SIX lanes (Engine · Bridge · History · **Housing Security** · Librarian · Foundry) and does **NOT** contain `INTEGRATOR`, `HOUSINGTEMP`, `HOUSING`, or `HOUSING/ENGINE`. But `INTEGRATOR` is a heavily-active lane and Foundry's `file_row.py` lint hardcodes `…HOUSING · HOUSINGTEMP · INTEGRATOR…` as canonical. So **LANES.md is STALE** (INTEGRATOR spun up post-roster like Foundry was) and the lint enforces a *different, hardcoded* set than the authority it should read from. **MY ERROR EXPOSED:** at CALLS row 81 I normalized a `HOUSINGTEMP` token → `HOUSING/ENGINE` on the Integrator's "it's a typo" claim — but LANES.md lists NEITHER token and Foundry now treats HOUSINGTEMP as a real lane. I ruled a token on an unverified lane claim WITHOUT checking the authority first; possibly mis-routed a real lane. **ROSTER CONTENT is the USER's ("a name on a face, the user said it") — routed to user.** Then: make LANES.md current + have the lint READ the roster from it (one source of truth), not a hardcoded copy. | ROUTED TO USER (roster content) + Foundry (lint reads LANES.md) — NOT inscribed |
| ROW-EXPANSION-DAMAGE | LINT DOCTRINE | Rows filed through an **UNQUOTED heredoc** get shell-expanded — backticked symbol names (esp. mangled `?name@@YA…@Z`) EXECUTE and vanish, `$vars` expand, backslashes collapse. **5 eaten rows in one day** (Housing/Foundry §947/§951); sibling of [[shell-escape-hazard]] (same root, backslash-collapse symptom, cost 2 rounds on the WW dialogue catalog). **Authoring-guide line LANDED by Librarian** in CALLS.md Protocol §"SAFE ROW FILING": quote the delimiter (`<< 'EOF'`) or write via Python `open('ab').write(...)`, then re-read the artifact. **Detector (flag a row with an unterminated/vanished span, or diff row-as-filed vs row-as-intended) = Bridge/Foundry's to build.** | AUTHORING-GUIDE LINE LANDED / detector routed to Bridge·Foundry |
...
**Blocker (verified counts):** the "§9 with 18 rows" the user means is the **frozen `WW-Restoration-Cookbook-SUPERSEDED.md` fork §9 "Failure classes" = 17 rows** (DO-NOT-WRITE). The **live** home is CANONICAL's "DIRECT-PORT CRASH RECIPES" = **9 recipes** (CANONICAL consolidated the old 17-class list into 9). So "recipes 1–9" currently matches the *live* count; re-pointing to §9 would cite frozen/superseded content. Also `§9` is overloaded (census "Spec §9" in `census_axis_*.py` is unrelated).
