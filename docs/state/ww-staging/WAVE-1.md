...
- **Foundry (USER RULING ferried, tale §939)**: PLUGIN IS THE PRODUCT + feasibility-first. **Mod-side only from now: no new files.cmake WW entries, no new native-TP seams; existing code not reverted.** Foundry's ordered first step = Tier-2 seam classification (A added-function / B in-function edit / C static data table); class-C count decides reachability.
- **Foundry (Tier-2 feasibility, tale §942)**: tier2_census.py — A 72 / B 299 / **C 15, ALL in d_stage.cpp's l_objectName**; its lookups dStage_searchName/getName are ordinary functions, so full plugin-ization is MECHANICALLY REACHABLE conditional on one hook proof (Housing 19a/19b). Self-caught: substring false positives (C 21→15) AND a shell-injected 0x08 corruption that moved counts invisibly — repaired byte-level before publishing.
- **Foundry (§942 CORRECTED + build identity, tale §945)**: class C is NOT unhookable — SDK has HOOK_CONTINUE/SKIP_ORIGINAL pre-hooks, so l_objectName rows LEAVE the array via lookup interception; **no known unhookable seam remains**. Lost-PDB root cause: GUID identical, age 817→844 — incremental link OVERWRITES in place, so archiving must run AT BUILD TIME. build_identity.py shipped (archive/resolve/check/list, refuses wrong-PDB symbolication). Self-corrected my §939 'zero receiver headers' clause per the user's delivery-vs-content clarification.
- **Foundry (§747 chain measured, tale §947)**: ALL THREE touchpoints hookable — proc name is an opaque s16 (no X-macro extension needed), profile slot hooks `fpcPf_Get` (**already specced in-tree: 'in the plugin it becomes a hook on this same function'**), placement route hooks dStage_searchName/getName with fall-through. **No measured blocker remains** between tree and the stated product — cost + one binder-reachability proof (3 named symbols for Housing).
- **Foundry (19a answered, tale §953)**: claimed per posture doc:329; "the real long pole" = **ZERO real symbols** — all 7 unresolvable (c)-set entries are artifacts (STL members, a LOCAL VARIABLE sp54, three COMMENT-PROSE matches, console-REL ModuleEpilog absent entirely); the 2 genuine host symbols both resolve. With §947+§948: **no measured technical blocker to the stated product** — only cost + first-use risk. binding_plan.py gains a declared ARTIFACT_CLASSES table with per-name reasons.
- **Foundry (headroom census, tale §954)**: profile_headroom.py — **13 FREE slots** (829 entries = 829 enum names, 796 profiled, 20 WW-owned) so the cDyl_ guard is hygiene TODAY, but **13 vs 46 unported Outset actors = a countdown**, load-bearing once consumed. Own parse error caught pre-publish (read the __MWERKS__ console array, not the PC one). 20-vs-33 WW-owned discrepancy FLAGGED not reconciled — boot manifest settles it.
- **Foundry (19b vs vanilla upstream, tale §957)**: upstream_conformance.py — **ALL 6 binding targets RESOLVE on stock dusklight** (28,956 exports + **272,796-entry embedded manifest — by-name resolution is a STOCK feature**); read-only, no clone, mods folder untouched. Converges with the Integrator's live clean-room PASS. Caught 2 of my own errors: checked the wrong surface (nearly published MISSING), and binding_plan's endswith('::name') is CLASS-BLIND (getManager matched 10 classes).
- **Foundry (silent-failure gate, tale §958)**: `build_identity.py manifest [exe]` — one command says DEAD or AVAILABLE for by-name resolution on any dusklight.exe (Housing's cmake quiet-path risk). Run on both: our fork FB887B64…-844 / 279,447 entries · upstream 7A48B338…-2 / 272,796 entries — **two unrelated build ids, both resolvable: version-independence DEMONSTRATED, not argued.**
- **Foundry (audit answered, tale §959)**: discrepancy DIAGNOSED — parser right, LABEL wrong: dict collapses 15,303 duplicate names; header count 294,750 now matches runtime EXACTLY (vanilla 288,472). **Safety finding: 5,076 names are AMBIGUOUS for by-name hooking** (empty string ×2,332, RES_U32 ×685) — the 3 migration targets each occur exactly once. Declined the offered fix of deleting the count; exposed both numbers + the ambiguity instead.
- **Foundry (hook-name safety, tale §960)**: `--symbol <name>` — SAFE/AMBIGUOUS/MISSING on BOTH fork and vanilla, separating the two ambiguity shapes (RES_U32 = 685 entries one name; getManager = 10 classes). Self-caught FALSE SAFE in v1 (queried the collapsed dict — the very trap §959 diagnosed); fixed via occurrences(). Named my recurring failure mode: measuring a DERIVED view and reporting it as the SOURCE (4x today).
- **Foundry (scoping accepted, tale §961)**: clean-room = SERVICE-path evidence (our plugin has ZERO hooks — verified); hook path has NO runtime proof. Ran --symbol on real B-class hosts: 2 SAFE both, 2 MISSING upstream — and chasing those exposed MY census defect: 'vanilla TP file' decided by FILENAME, so our own functions in upstream-stub files scored as class B (3 of 12 sampled, 27 of 89 sites). §942's B=299 likely OVERSTATES; re-run needed with HOST-EXISTS-UPSTREAM as the definition.
- **Foundry (B-class re-run, tale §962)**: full 148-host pass with B redefined as HOST-EXISTS-UPSTREAM — **real hook workload 221 sites / 105 hosts, not 299**; 43 hosts (78 sites) are OUR functions in upstream-stub files and migrate with the plugin. §942's B=299 superseded; price from 221/105. UNKNOWN (never filename-fallback) when the upstream image is unavailable.
- **Foundry (hook-surface audit, tale §963)**: receiver-layer ambiguity = **12 of 73 hookable hosts (~16%)**, not 55% — first run matched BARE names (5th derived-view error, caught pre-publish); offenders named (useHeapInit x196, dComIfGs_onDungeonItem*). 28 split-verdict hosts are all OUR ports, not vanilla hosts. **§962's 221/105 marked SUSPECT** — it used loose matching; genuine population is 73 not 105, and I did NOT replace the number.
- **Foundry (SUSPECT number resolved, tale §964)**: qualified-name matching lands the final split — **hook workload = 140 sites / 73 hosts** (§942's 299 and §962's 221 both SUPERSEDED); **53% of the original count was never hook work** — our own ports counted as vanilla hosts, first via filenames, then via bare-name matching. Price from 140/73; 12 of the 73 need address-hooks per §963.
- **Foundry (ambiguity split, tale §965)**: of §963's 12 ambiguous hosts — **9 FIXABLE by requiring mangled names** (dComIfGs_on* family, phase_2, fopMsgM_messageSet; clean overload pairs) and **3 IRREDUCIBLE** (useHeapInit x2 against 198 shared entries, phase_4) = **3 hosts / 8 sites, ~5.7% of the 140-site hook workload**, needing address- or caller-hooks. Mangled-names-required is worth a rule but must NOT be the only check (it would pass useHeapInit as SAFE).
- **Foundry (sibling-tool fix, tale §966)**: `symbol_manifest.py --find` carried §960's collapsed-dict false SAFE for six sections — shape A (685/198 raw entries under one name) reported plain RESOLVED while the sibling said AMBIGUOUS; **the flag the Integrator cited as a gate**. Ported occurrences(), verified on 3 controls + 1 clean. NEW lesson class: not a derived-view error but a FIX THAT DID NOT PROPAGATE — sweep every caller of a trap before closing the row. Their LOCAL-flag finding (statics have no mangled form) explains WHY §965's 3 are irreducible.
- **Foundry (trap sweep, tale §967)**: applied §966's lesson — swept all 4 callers of the collapsed dict, found 2 more failing optimistically. **binding_plan verdicts FIXED** (third state 'ambiguous'; getManager/RES_U32/useHeapInit were all 'resolved'); **fold_detect exposure MEASURED + printed** (15,303 entries never reach its map — counts are a FLOOR; declined to rewrite unread logic). Housing's parallel read makes the irreducible set **ZERO** (all 3 are callbacks via uniquely-named entry points). Hook surface final: 73 hosts, 9 need mangled names, 0 unsolvable.
- **Foundry (slot scarcity dissolved, tale §968)**: `fpcNm_MAX_NUM` bounds exactly TWO things — fpcPf_Get's guard (already exists, §706) and DMC[]; on PC only `cDyl_IsLinked` can wild-read a plugin index, and it is SAFE-hookable in BOTH images. **Two hooks buy unlimited indices — no slot budget, no receiver edit.** Decisive: every slot-expansion option requires editing the USER's exe, so it is incompatible with the product, not merely worse. Lane: cDyl hook + f_pc/c_dylink ownership → HOUSING; verification stays Foundry.
- **Foundry (§968 corrected pre-build, tale §969)**: I specified the cDyl_IsLinked hook BACKWARDS — `fpcLd_Use` gates actor creation on it (f_pc_fstcreate_req.cpp:31) and the PC load half always succeeds, so returning FALSE would make every above-enum WW actor SILENTLY not spawn. **Hook must return TRUE**, reproducing the function's own NULL branch. Root: reasoned from the symbol, not the call path — second time (cf. §967). Also softened 'impossible' → 'impossible without shipping a modified exe' (DMC size also drives a heap alloc).
- **Foundry (consumer trace on the unwritten hooks, tale §970)**: applied §969's rule to every specced target before code exists. **`dStage_getName` is DEBUG-ONLY — all consumers are OS_REPORT/dDbVw_Report, nothing branches on it, so it does not need hooking; functional set is TWO.** `dStage_searchName` returns a POINTER — no consumer caches it, but it must outlive the call (plugin-owned static, never a stack temporary), and d_event_data.cpp:977 ASSERTS non-null so the hook must fall through rather than answer NULL. `fpcPf_Get` clean. All four verified unambiguous on BOTH images.
- **Foundry (receipt check built, tale §975)**: `call_receipt.py` — the attribution blocker did NOT need solving; addressed-lane prefix is already uniform, § is a monotonic clock, bus headers give liveness (5 lanes vs 2). **No format migration asked of any lane.** Caught 3 false-positive classes pre-ship incl. calling the INTEGRATOR unreachable (liveness must be cross-bus even though the clock is per-bus). **Housing's append caution was a real hole and is fixed**: an append citing a later § than its parent cannot be covered by the parent's [x] — 4 found, one a live ask on a checked row. Live board: 2 unreceived (HISTORY@§903, HOUSINGTEMP@§697), LIBRARIAN UNKNOWN on both buses.
- **Foundry (WITHDRAWING a false accusation + both audits acted, tale §977)**: my §975 row said HISTORY was silent since §903 — **false**, they are at §976 and have worked throughout; HOUSINGTEMP also rescued. Receipt check now reports ZERO unreceived. **Root: a receipt verdict is a SNAPSHOT of a moving board and I filed it as a standing row — new rule: RUN it, don't FILE it; if written, carry the measurement §.** Housing found the real bug (tool flagged rows the addressee AUTHORED); Integrator's 3 audit points acted incl. new `--why <LANE>` that shows every input behind a verdict — which is what caught my own false accusation. **Still no lane has to change its attribution format**: every heuristic is rescue-only, so an over-match suppresses a finding and can never invent one.
- **Foundry (allocator CLEARED; 39 unallocated sections all one lane)**: §976 was not a double-issue — the ledger issued it once (Librarian, now §976b); the other §976 was never allocated though its header CLAIMS it was. `staging.py verify`: **39 UNALLOCATED + 26 COLLISIONS, and all 39 unallocated are History's** (ledger has zero History rows in §960-977). Allocator's own race guard is sound; the gap is upstream. §958 and §970 collide with allocated FOUNDRY sections and §970 is already cited — renumber pre-citation.
- **Housing/Engine (by-name resolution is EMBEDDED, CALLS row 115)**: `HookService::resolve` does NOT read `dusklight_exports.def` — `manifest::initialize` (src/dusk/mods/manifest.cpp:242) reads a symgen blob at `imageBase + rva`, a section INSIDE the running exe embedded by a POST_BUILD step and validated against that image's own build id. **So a `DEFINE_HOOK_SYMBOL` plugin bakes no addresses and ships no symbol file — it is version-independent by construction, which is the mechanism behind "download the plugin, point it at your ISO".** The one gate: `rva == 0` ⇒ "by-name resolution unavailable" and EVERY plugin hook fails silently — `DUSK_ENABLE_CODE_MODS` defaults ON but SymbolManifest.cmake has a quiet `return()` when no symgen prebuilt exists for the host. Foundry turned this into `build_identity.py manifest` (§958). Stale comment at manifest.hpp:26/36 corrected.
- **Housing/Engine (hook path proven, `ww_hook_smoke`, run 180329)**: built the 11-discriminator first-use probe; the Integrator ran it on **vanilla upstream**. Both name forms resolve to the SAME address, ambiguity 0/0 (pre-registered in the emitted line), `add_pre`/`add_post` MOD_OK, detour FIRES with real payloads, `ret_nonnull:1` on every post (HOOK_CONTINUE holds), 46/46 and 120/120 balance. **H10 answered from the artifact: `dumpbin /imports` shows NO dusklight.exe import — `FEATURES game` is a build-order coupling only.** Instrument, retire when banked.
- **Housing/Engine (the registration VEHICLE, user ruling §968 option 1, CALLS row 234)**: `mods-src/ww_registry/` — two hooks buy unlimited indices. `fpcPf_Get` answers for our range; `cDyl_IsLinked` returns **TRUE** (reproducing the array's own `d == NULL` branch — returning "not linked" would have made every WW actor silently fail to spawn, corrected pre-build with §969). Range test is against the PLUGIN'S OWN base (4096), not `fpcNm_MAX_NUM`, so it cannot mis-fire on a dusklight whose proc list grew. Gate 0-B clean, no receiver edit. **Run 183312 on vanilla: both hooks installed, `pf_hits:0` — provably inert.** Dispatch self-test since landed (sentinel at 4096, called through the trampolined address) to close the one gap that run named. **The 13-slot ceiling and the 46-actor Outset countdown are gone.**
