...
provenance:    see-file
citations:
  - src/d/d_event_manager.cpp KIT-LINEAGE host-plumbing (read 2026-08-16)
...
notes: |
  ...
  exists to host donor content. Recorded distinctly from `native-port` because the
  two are different provenances that happen to share a destination - collapsing
  them would lose which side authored the code.
