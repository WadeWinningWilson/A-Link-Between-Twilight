...
| **FARON TWILIGHT BORDER EARLY (Talo quest) — FIXED 2026-08-28** | **SYMPTOM:** fresh save, first Faron visit during save-Talo (`F_SP104` / woods path) — twilight barrier (`twGtFiro` + `TgTGate`) visible too early. **LOG:** `dusklight-20260828-131821.log` — `fpcNm_Obj_TwGate_e` + `fpcNm_Tag_TWGate_e` created ~gFrm 711 on `F_SP104` room 1 (`m_tgate_firone`), repeats ~gFrm 24431. **ROOT CAUSE:** `d_albw_twilight_border` fallback gated only on `!isDarkClearLV(province)`; pre-spread Faron is uncleared but twilight has not reached the province yet. Vanilla uses `dKy_F_SP121Check`: Faron twilight blocked until Ordon Day 2 (`0x4510`). **FIX:** `isProvinceTwilightSpread()` — Faron requires `0x4510`; Eldin/Lanayru/Hyrule borders require prior province cleared. Playtest: Talo chase → no barrier at Ordon Spring; post–Day-2 twilight approach → fallback still restores missing day-layer borders. |
