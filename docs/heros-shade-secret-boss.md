...
- **TODO next per roadmap: BOSS HEALTH BAR** — a NEW bar (green bar in screenshots is the PLAYER's ALBW meter, NOT the boss), to appear ONLY under a new "Boss Health bar" setting (ConfigVar + editor toggle). Then defeat/farewell event → warp back to basement wolf; 3-phase structure (70/40/0%) + armor-shed + desync tell. Then more attacks (jump strike, spin, mortal draw, clones) + parry-stagger. Point-blank sphere reposition.
