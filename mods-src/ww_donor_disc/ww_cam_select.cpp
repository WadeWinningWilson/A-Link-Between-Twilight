...
void camlog(LogLevel level, const char* fmt, ...) {
    if (s_log == nullptr) {
        return;
    }
    char buf[512];
...
WwCamSelState s_st;
dCamera_c* s_attachedCam = NULL;
int s_inSelectLatch = 0;
...
void logTransition(dCamera_c* i_cam, const char* i_what, int i_type, int i_dMode, int i_style) {
    ...
    camlog(LOG_LEVEL_INFO,
           "[WwCamSel] %s: type %d '%s' dMode %d style %d (%c%c%c%c) alg %d", i_what, i_type,
           wwType(i_type).name, i_dMode, i_style, (char)((id >> 24) & 0xFF),
...
void attach(dCamera_c* i_cam) {
    ...
    // Vanilla indexes engine_tbl[Algorythmn] with no extra-engine hook, and
    // Algorythmn is an inline field read. Alg 20+ is OOB. CRAWL (20) occupies
    // test2 (19); unported sockets 21-27 occupy test1 (18), which returns
    // false — same hold-last-frame as the fork extra-engine decline.
    const int nStyles = dExtWwCam_styleCount();
    for (int i = 0; i < nStyles; i++) {
        const int alg = (int)i_cam->mCamParam.mCamStyleData[i].field_0x4;
        if (alg == 20) {
            i_cam->mCamParam.mCamStyleData[i].field_0x4 = 19;
        } else if (alg >= 21 && alg <= 27) {
            i_cam->mCamParam.mCamStyleData[i].field_0x4 = 18;
        }
    }
    ...
    s_attachedCam = i_cam;
    camlog(LOG_LEVEL_INFO,
           "[WwCamSel] attached: %d styles / %d donor-axis types live; default type '%s'",
...
void detach(dCamera_c* i_cam) {
    ...
    if (s_attachedCam != NULL && s_attachedCam != i_cam) {
        s_st.attached = false;
        s_attachedCam = NULL;
        camlog(LOG_LEVEL_INFO, "[WwCamSel] detached: camera instance changed, skip restore");
        return;
    }
    ...
    s_st.attached = false;
    s_attachedCam = NULL;
    camlog(LOG_LEVEL_INFO, "[WwCamSel] detached: receiver camera data restored");
}
...
fopAc_ac_c* boomerangActor(dCamera_c* i_cam) {
    ...
    if ((fopAc_ac_c*)link == i_cam->mpPlayerActor && link != NULL) {
        return static_cast<fopAc_ac_c*>(daPy_py_c::getThrowBoomerangActor());
    }
    return NULL;
}
...
bool wwCamSelect(dCamera_c* i_cam) {
    if (!wwRegistry_isWwHostStage()) {
        if (s_st.attached) {
            detach(i_cam);
        }
        return false;
    }
    if (s_st.attached && s_attachedCam != i_cam) {
        s_st.attached = false;
        s_attachedCam = NULL;
    }
    if (!s_st.attached) {
    ...
    if (nextType != i_cam->mCurType && nextType >= 0) {
        ...
        if (s_st.m144 == 0) {
            ...
            if (style0 >= 0) {
                ...
                case 19:  // CRAWL on vanilla test2 socket (was extra-engine 20)
                case 11:  // hookshot
                    s_st.dMode = 0;
                    s_st.m144 = 1;
                    break;
    ...
    if (style >= 0 && i_cam->mCamStyle != style) {
        if (i_cam->mCamStyle >= 0) {
            ...
            case 4: case 5: case 6: case 11: case 19:
                if (s_st.m144 == 0) {
                    s_st.m144 = 1;
                }
                break;
...
bool wwCam_isAttached() {
    return s_st.attached;
}

bool wwCam_inSelectLatch() {
    return s_inSelectLatch != 0;
}
