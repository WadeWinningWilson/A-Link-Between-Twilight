...
// ============================================================
// §254 WW Tetra (NPC_ZL1) DIRECT PORT — header VERBATIM from the WW donor
// (D:/XXXXXXX/WW DP/include/d/actor/d_a_npc_zl1.h). No offsets changed. Base
// class fopNpc_npc_c and every dNpc_* / message helper are NATIVE in the port
// (include/d/d_npc.h — the same framework the Aryll §244 / bm1 §246 ports use).
// The only header edit vs the donor is the node-callback rename J3DNode*->J3DJoint*
// (the port's J3DJointCallBack is int(*)(J3DJoint*,int); match the pointer token
// only — do NOT substring-replace J3DNode). All member types (LIGHT_INFLUENCE,
// dPa_rippleEcallBack, mDoExt_offCupOnAupPacket/onCupOffAupPacket, mDoExt_btkAnm/
// btpAnm, mDoHIO_entry_c, request_of_phase_process_class) exist in the port; the
// only shim-sourced member type is dPa_rippleEcallBack (d_ext_ww_actor_shims.h).
// ============================================================

#include "d/d_npc.h"
#include "d/d_kankyo.h"                 // §254 LIGHT_INFLUENCE (omamori glow)
#include "m_Do/m_Do_ext.h"             // §254 mDoExt_offCupOnAupPacket/onCupOffAupPacket + btk/btp anm
#include "m_Do/m_Do_hostIO.h"          // §254 mDoHIO_entry_c (HIO param table)
#include "f_op/f_op_actor.h"
#include "SSystem/SComponent/c_phase.h"
#include "d/d_ext_ww_actor_shims.h"    // §254 dPa_rippleEcallBack (port has it only as a shim)
...
public:
    ...
    void _nodeCB_Head(J3DJoint*, J3DModel*);
    void _nodeCB_BackBone(J3DJoint*, J3DModel*);
    ...
    BOOL _execute();
    BOOL _delete();
    cPhs_Step _create();  // §254 WW cPhs_State -> port cPhs_Step
    BOOL bodyCreateHeap();
    BOOL itemCreateHeap();
    BOOL CreateHeap();
...
public:
    ...
    // §254 the donor's 4 eye-layering packets (mDoExt_offCupOnAupPacket /
    //   onCupOffAupPacket) are by-value members here, but the port declares their
    //   destructors PRIVATE (m_Do_ext.h) -> a by-value member deletes daNpc_Zl1_c's
    //   own destructor -> it can't override fopNpc_npc_c's virtual dtor. Since the
    //   cup/aup layered _draw is a DEFERRED bridge (no port entryOpa; see .cpp),
    //   these are replaced with equivalent padding to keep the 0x900 layout.
    /* 0x8C0 */ u8 field_0x8C0[0x900 - 0x8C0];
};  // Size: 0x900
