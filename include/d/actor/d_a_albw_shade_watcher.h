#ifndef D_A_ALBW_SHADE_WATCHER_H
#define D_A_ALBW_SHADE_WATCHER_H

#include "d/actor/d_a_npc4.h"

struct daAlbwShadeWatcher_HIOParam {
...
class daAlbwShadeWatcher_Param_c {
public:
    virtual ~daAlbwShadeWatcher_Param_c() {}

    static daAlbwShadeWatcher_HIOParam const m;
};

#if DEBUG
class daAlbwShadeWatcher_HIO_c : public mDoHIO_entry_c {
public:
    daAlbwShadeWatcher_HIO_c();

    void listenPropertyEvent(const JORPropertyEvent*);

    void genMessage(JORMContext*);

    /* 0x8 */ daAlbwShadeWatcher_HIOParam m;
};

#define ALBW_SHADE_WATCHER_HIO_CLASS daAlbwShadeWatcher_HIO_c
#else
#define ALBW_SHADE_WATCHER_HIO_CLASS daAlbwShadeWatcher_Param_c
#endif

/**
 * @ingroup actors-npcs
 * @class daAlbwShadeWatcher_c
 * @brief Golden Wolf
 *
 * @details
 *
*/
class daAlbwShadeWatcher_c : public daNpcF_c {
public:
    typedef BOOL (daAlbwShadeWatcher_c::*actionFunc)(void*);
    typedef BOOL (daAlbwShadeWatcher_c::*cutFunc)(int);

    daAlbwShadeWatcher_c();
    cPhs_Step create();
    int CreateHeap();
    int Delete();
    int Execute();
...
private:
    ...
    /* 0xC88 */ ALBW_SHADE_WATCHER_HIO_CLASS* mpHIO;
    /* 0xC8C */ dCcD_Cyl mCyl;
...
STATIC_ASSERT(sizeof(daAlbwShadeWatcher_c) == 0xe20);

#endif /* D_A_ALBW_SHADE_WATCHER_H */
