...
// ============================================================
// §253 WW Rito Postbox (OBJ_TORIPOST) DIRECT PORT — header VERBATIM from the WW
// donor (D:/XXXXXXX/WW DP/include/d/actor/d_a_obj_toripost.h). No offsets changed.
// Base class fopNpc_npc_c and every dNpc_* / message helper are NATIVE in the port
// (include/d/d_npc.h — same file the Aryll §244 port uses). The .cpp applies the
// pig/seagull direct-port crash recipes (DN-3 acquireModelData, render triple,
// (uintptr_t)this, cPhs_State->cPhs_Step) at the call sites.
//
// NOTE (base class): the task brief called this an fopAc_ac_c obj-tier actor, but
// the donor header + body are unambiguously fopNpc_npc_c (talk()/eventInfo/mEventCut/
// mpCurrMsg/next_msgStatus/getMsg). Ported donor-faithful as fopNpc_npc_c.
// ============================================================
...
public:
    ...
    void createInit();
    void getArg();
    cPhs_Step _create();  // §253 WW cPhs_State -> port cPhs_Step
    bool _delete();
